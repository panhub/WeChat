...
- (void)handParameters {
    ...
    NSString *parameter = [dictionary queryValue];
...
- (void)didLoadFinishWithResponse:(MNURLResponse *)response {
    ...
    NSInteger retCode = [NSDictionary integerValueWithDictionary:data forKey:@"retCode"];
    response.message = [NSDictionary stringValueWithDictionary:data forKey:@"msg" def:@"请求失败"];
    if (retCode == 200) {
