...
- (void)setModel:(WXChangeModel *)model {
    ...
    self.detailLabel.textColor = model.money > 0.f ? MN_R_G_B(238.f, 176.f, 38.f) : [UIColor blackColor];
    [self.detailLabel sizeToFit];
    self.detailLabel.right_mn = self.contentView.width_mn - self.titleLabel.left_mn;
    
    self.timeLabel.text = [NSDate stringValueWithTimestamp:model.timestamp format:@"MM月dd日 HH:mm"];
}
