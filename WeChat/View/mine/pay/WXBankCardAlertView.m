...
- (void)loadData {
    NSArray <WXBankCard *>*array = [[NSArray alloc] initWithArray:[[WechatHelper helper] cards]];
...
- (void)createView {
    ...
    UILabel *titleLabel = [UILabel labelWithFrame:CGRectMake(closeButton.right_mn + 10.f, 0.f, contentView.width_mn - (closeButton.right_mn + 10.f)*2.f, 18.f) text:@"" alignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor darkTextColor], .9f) font:UIFontRegular(18.f)];
    titleLabel.centerY_mn = closeButton.centerY_mn;
    [contentView addSubview:titleLabel];
    self.titleLabel = titleLabel;
    
    UILabel *detailLabel = [UILabel labelWithFrame:CGRectMake(titleLabel.left_mn, titleLabel.bottom_mn + 10.f, titleLabel.width_mn, 14.f) text:@"" alignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor darkTextColor], .5f) font:UIFontRegular(14.f)];
    [contentView addSubview:detailLabel];
    self.detailLabel = detailLabel;
    
    UIImageView *shadow = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"moment_more_line"]];
    shadow.frame = CGRectMake(0.f, detailLabel.bottom_mn + titleLabel.top_mn, contentView.width_mn, .8f);
    shadow.hidden = YES;
    [contentView addSubview:shadow];
    self.shadow = shadow;
    
    UITableView *tableView = [UITableView tableWithFrame:CGRectMake(0.f, shadow.bottom_mn, contentView.width_mn, 1000.f) style:UITableViewStylePlain];
    ...
    separator.frame = CGRectMake(0.f, footerButton.height_mn - 1.f, footerButton.width_mn, .8f);
    [footerButton addSubview:separator];
...
- (void)layoutSubviews {
    ...
    self.contentView.height_mn = self.tableView.bottom_mn + MN_TAB_SAFE_HEIGHT;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        cell.bottomSeparatorInset = UIEdgeInsetsMake(0.f, cell.imgView.left_mn, 0.f, 0.f);
    }
    return cell;
}

- (void)tableView:(UITableView *)tableView willDisplayCell:(WXBankCardAlertViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    cell.card = self.dataArray[indexPath.row];
    cell.withdraw = self.type == WXBankCardAlertViewWithdraw;
    cell.topSeparatorInset = indexPath.row == 0 ? cell.bottomSeparatorInset : UIEdgeInsetsMake(0.f, 0.f, 0.f, self.contentView.width_mn);
}
