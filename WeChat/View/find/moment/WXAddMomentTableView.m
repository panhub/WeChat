...
- (void)loadData {
    ...
    model3.title = @"发布时间";
    model3.desc = [MNChatHelper momentCreatedTimeWithTimestamp:self.viewModel.timestamp];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        vc.didSelectHandler = ^(WXMapLocation *location) {
            @strongify(self);
            NSString *string = [NSString replacingBlankCharacter:location.name];
        ...
        [[MNDatePicker datePickerWithHandler:^(MNDatePicker *datePicker) {
            ...
            model.desc = [MNChatHelper momentCreatedTimeWithTimestamp:self.viewModel.timestamp];
