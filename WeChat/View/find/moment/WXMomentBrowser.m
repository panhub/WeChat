...
- (instancetype)initWithViewModel:(WXMomentViewModel *)viewModel {
    if (self = [super init]) {
        ...
        if (viewModel.moment.isMine) {
            MNNavigationBar *browseBar = [[MNNavigationBar alloc] initWithFrame:CGRectMake(0.f, 0.f, self.width_mn, MN_TOP_BAR_HEIGHT) delegate:self];
            ...
            browseBar.shadowView.backgroundColor = UIColor.clearColor;
            browseBar.backgroundColor = MN_RGB(46.f);
            browseBar.alpha = 0.f;
        ...
        if (viewModel.moment.profiles.count > 1) {
            MNPageControl *pageControl = [[MNPageControl alloc] initWithFrame:CGRectMake(0.f, 0.f, self.width_mn, 20.f)];
            pageControl.alpha = 0.f;
            pageControl.numberOfPages = viewModel.moment.profiles.count;
            pageControl.currentPageIndicatorTintColor = [UIColor whiteColor];
            pageControl.userInteractionEnabled = NO;
            pageControl.bottom_mn = self.height_mn - MN_TAB_SAFE_HEIGHT - 10.f;
