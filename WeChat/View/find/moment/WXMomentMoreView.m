...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [titles enumerateObjectsUsingBlock:^(NSString * _Nonnull title, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [button.titleLabel setFont:UIFontRegular(15.f)];
            ...
            /// 分割线
            UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(interval + (interval + .5f)*idx, MEAN(WXMomentMoreViewHeight - 23.f), .5f, 23.f)];
            imageView.hidden = idx > 0;
            imageView.clipsToBounds = YES;
            imageView.contentMode = UIViewContentModeScaleAspectFill;
            imageView.image = UIImageNamed(@"wx_moment_moreview_line");
