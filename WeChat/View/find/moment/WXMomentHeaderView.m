...
- (void)setViewModel:(WXMomentViewModel *)viewModel {
    ...
    self.pictureView.pictures = viewModel.moment.profiles;
    // 分享
...
- (void)handEvents {
    ...
    /// 配图点击事件
    self.pictureView.touchEventHandler = ^(NSArray<MNAsset *> *assets, NSInteger index) {
        @strongify(self);
    ...
    [self.moreButton handEvents:UIControlEventTouchUpInside eventHandler:^(id sender) {
        ...
        if ([[MNConfiguration configuration] keyboardVisible]) {
            ...
            self.moreView.liked = [self.viewModel.moment.likes.copy filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self.uid == %@", WXUser.shareInfo.uid]].count > 0;
    ...
    self.moreView.eventHandler = ^(NSUInteger idx) {
        @strongify(self);
        if (self.viewModel.moreViewEventHandler) {
            self.viewModel.moreViewEventHandler(self.viewModel, idx);
        }
    };
