...
- (instancetype)initWithReuseIdentifier:(NSString *)reuseIdentifier size:(CGSize)size {
    if (self = [super initWithReuseIdentifier:reuseIdentifier size:size]) {
        ...
        UILabel *signatureLabel = [UILabel labelWithFrame:UIEdgeInsetsInsetRect(signatureView.bounds, UIEdgeInsetsMake(0.f, 5.f, 0.f, 5.f)) text:nil alignment:NSTextAlignmentCenter textColor:self.detailLabel.textColor font:UIFontRegular(12.f)];
...
- (void)setHistory:(WXShakeHistory *)history {
    ...
    if (history.gender == WechatGenderUnknown) {
        ...
        self.genderView.image = [UIImage imageNamed:(history.gender == WechatGenderMale ? @"wx_contacts_gender_male" : @"wx_contacts_gender_female")];
        self.genderView.hidden = NO;
    }
    if (history.signature.length) {
        CGFloat width = MIN([NSString stringSize:history.signature font:self.signatureLabel.font].width, 90.f);
