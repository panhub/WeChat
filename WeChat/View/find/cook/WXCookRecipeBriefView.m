...
+ (instancetype)viewWithRecipeModel:(WXCookRecipe *)model {
    WXCookRecipeBriefView *headerView = [[WXCookRecipeBriefView alloc] initWithFrame:CGRectMake(0.f, 0.f, SCREEN_WIDTH, 0.f)];
...
- (void)setModel:(WXCookRecipe *)model {
    ...
    if (model.sumary.length) {
        ...
        UILabel *sumaryLabel = [UILabel labelWithFrame:CGRectMake(12.f, 15.f, size.width, size.height) text:sumary textAlignment:NSTextAlignmentLeft textColor:nil font:nil];
    ...
    [model.ingredients enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        UILabel *ingredientLabel = [UILabel labelWithFrame:CGRectMake(12.f, (idx == 0 ? (height + 15.f) : (height + 7.f)), size.width, size.height) text:ingredients textAlignment:NSTextAlignmentLeft textColor:nil font:nil];
