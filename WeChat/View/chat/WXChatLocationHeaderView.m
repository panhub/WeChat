...
- (void)createView {
    ...
    MAMapView *mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0.f, 0.f, self.contentView.width_mn, (MN_SCREEN_HEIGHT - MN_STATUS_BAR_HEIGHT - MN_NAV_BAR_HEIGHT*2.f)/2.f)];
...
- (void)takeSnapshotCompletion:(void(^)(UIImage *))handler {
    ...
    [self.mapView takeSnapshotInRect:rect withCompletionBlock:^(UIImage *resultImage, NSInteger state) {
        if (handler) handler(state == 1 ? resultImage : nil);
    }];
}
