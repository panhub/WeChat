...
- (void)createView {
    ...
    CGFloat x = MEAN(contentView.width_mn - record_size.width - volume_size.width - 10.f);
    CGFloat y = MEAN(contentView.height_mn - record_size.height - 25.f);
    ...
    UILabel *hintLabel = [UILabel labelWithFrame:CGRectMake(10.f, recordBG.bottom_mn, contentView.width_mn - 20.f, 25.f) text:@"手指上滑, 取消发送" textAlignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor whiteColor], .8f) font:[UIFont systemFontOfSize:16.f]];
    hintLabel.layer.cornerRadius = 3.f;
    hintLabel.clipsToBounds = YES;
    [contentView addSubview:hintLabel];
    self.hintLabel = hintLabel;
    
    UILabel *timeLabel = [UILabel labelWithFrame:CGRectMake(0.f, 0.f, contentView.width_mn, hintLabel.top_mn) text:@"5" textAlignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor whiteColor], .85f) font:[UIFont systemFontOfSize:contentView.width_mn/2.f]];
...
- (void)setState:(WXChatVoiceRecordState)state {
    ...
    if (state == WXChatVoiceRecordNormal) {
        ...
        self.hintLabel.backgroundColor = R_G_B(145.f, 50.f, 40.f);
