...
static MNChatHelper *_chatHelper;
...
NSString * MNRandomPhoneNumber (void) {
    ...
    NSMutableString *number = MNPhoneNumberPrefixSet[arc4random()%count].mutableCopy;
    for (int i = 0; i < 8; i++) {
        int random = arc4random()%9;
        [number appendString:NSStringFromNumber(@(random))];
    }
    return number;
}

@interface MNChatHelper ()
...
- (void)reloadData {
    ...
    if (self.sessionUpdateHandler) self.sessionUpdateHandler(self.sessions);
    /// 删除聊天, 朋友圈相关数据
    [MNChatHelper.helper.cache removeAllObjectsWithCompletion:nil];
...
- (void)needAddContactsNotification:(NSNotification *)notification {
    ...
    if (![MNDatabase.sharedInstance insertIntoTable:WXContactsTableName model:user]) return;
    [self.contacts addObject:user];
    if (self.contactsUpdateHandler) {
        self.contactsUpdateHandler(self.contacts);
    }
}
...
- (void)needAddSessionNotification:(NSNotification *)notification {
    ...
    dispatch_async_main(^{
        if (self.sessionUpdateHandler) {
            self.sessionUpdateHandler(self.sessions);
        }
    });
}
...
- (void)needBringSessionToFrontNotification:(NSNotification *)notification {
    WXSession *session = notification.object;
    if (!session) return;
    if ([MNDatabase.sharedInstance updateTable:WXSessionTableName where:[@{sql_field(session.identifier):session.identifier} componentString] model:session]) {
        if (session.front) {
...
- (void)needUpdateChangeMoneyNotification:(NSNotification *)notify {
    [MNDatabase selectColumnSumFromTable:WXChangeTableName field:@"money" completion:^(CGFloat sum) {
        dispatch_async_main(^{
...
- (void)fetchSystemContacts {
    [MNAddressBook fetchContactsWithCompletionHandler:^(NSArray<MNContact *> *contacts) {
        [contacts enumerateObjectsUsingBlock:^(MNContact * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            MNLabeledValue *phone = obj.phones.lastObject;
            WXUser *user = MNChatHelper.generateRandomUser;
            user.notename = obj.name;
            user.desc = @"系统通讯录";
            NSString *number = [phone.value stringByReplacingOccurrencesOfString:@"\\p{Cf}" withString:@""];
            ...
            [self.contacts addObject:user];
            [MNDatabase insertIntoTable:WXContactsTableName model:user completion:nil];
        }];
...
+ (UIImage *)randomAvatarImage {
    ...
    NSString *img = [NSString stringWithFormat:@"wx_avatar_%@", @(count)];
    return [[UIImage imageNamed:img] resizingToPix:300.f];
}
...
- (BOOL)insertUserToContacts:(WXUser *)user {
    ...
    if (![MNDatabase.sharedInstance insertIntoTable:WXContactsTableName model:user]) return NO;
    [self.contacts addObject:user];
    if (self.contactsUpdateHandler) {
        self.contactsUpdateHandler(self.contacts);
    }
...
- (void)asyncLoadSessions:(void(^)(void))completionHandler {
    dispatch_async(self.queue, ^{
        ...
        [MNDatabase selectRowsModelFromTable:WXSessionTableName class:[WXSession class] completion:^(NSArray<id> * _Nonnull rows) {
            /// 倒序添加
            [self.sessions addObjectsFromArray:[rows reverseObjects]];
            [self makeFrontConversations];
            [self asyncSessionToSandbox];
            dispatch_async_main(^{
                if (self.sessionUpdateHandler) self.sessionUpdateHandler(self.sessions);
            });
...
- (void)makeFrontConversations {
    ...
    [frontArray.reverseObjects enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (void)insertBankCard:(WXBankCard *)card completion:(void(^)(BOOL succeed))completion {
    dispatch_async([[MNChatHelper helper] queue], ^{
        BOOL succeed = [[MNChatHelper helper] insertBankCard:card];
        dispatch_async_main(^{
            if (completion) {
                completion(succeed);
            }
...
- (BOOL)insertBankCard:(WXBankCard *)card {
    ...
    if ([[MNDatabase sharedInstance] insertIntoTable:WXBankCardTableName model:card]) {
...
- (BOOL)deleteBankCard:(WXBankCard *)card {
    ...
    if ([[MNDatabase sharedInstance] deleteRowFromTable:WXBankCardTableName where:[@{sql_field(card.number):card.number} componentString]]) {
...
+ (void)deleteBankCard:(WXBankCard *)card completion:(void(^)(BOOL succeed))completion {
    dispatch_async([[MNChatHelper helper] queue], ^{
        BOOL succeed = [[MNChatHelper helper] deleteBankCard:card];
        dispatch_async_main(^{
            if (completion) {
                completion(succeed);
            }
...
+ (void)updateBankCard:(WXBankCard *)card completion:(void(^)(BOOL succeed))completion {
    dispatch_async([[MNChatHelper helper] queue], ^{
        BOOL succeed = [[MNChatHelper helper] updateBankCard:card];
        dispatch_async_main(^{
            if (completion) {
                completion(succeed);
            }
...
- (BOOL)updateBankCard:(WXBankCard *)card {
    if (card.isValid == NO) return NO;
    return [[MNDatabase sharedInstance] updateTable:WXBankCardTableName
...
+ (WXUser *)generateRandomUser {
    ...
    user.uid = NSDateShortTimestamps();
    user.notename = user.nickname = [NSString generateChineseWithLength:(arc4random()%3 + 2)];
    user.wechatId = MNChatHelper.generateRandomWechatId;
    user.number = MNRandomPhoneNumber();
    user.location = WXUserDefaultLocation;
    user.gender = arc4random()%(MNGenderFemale + 1);
    user.avatarData = MNChatHelper.randomAvatarImage.PNGData;
    if (arc4random_uniform(100) >= 50) user.signature = MNChatHelper.helper.signatures.randomObject;
    return user;
}
...
+ (NSString *)chatMsgCreatedTimeWithTimestamp:(NSString *)timestamp {
    ...
    if ((components.year + components.month) >= 1 || components.day >= 7) {
        return [NSDate dateStringWithTimestamp:timestamp format:@"yyyy年-MM月-dd日 HH:mm"];
    } else if (components.day == 0) {
        return [NSDate dateStringWithTimestamp:timestamp format:@"HH:mm"];
    } else if (components.day == 1) {
        return [@"昨天 " stringByAppendingString:[NSDate dateStringWithTimestamp:timestamp format:@"HH:mm"]];
    }
    return [NSString stringWithFormat:@"%@ %@", [NSDate weekdayFromDate:timestamp], [NSDate dateStringWithTimestamp:timestamp format:@"HH:mm"]];
}
...
- (NSMutableArray <WXBankCard *>*)cards {
    if (!_cards) {
        NSArray *rows = [[MNDatabase sharedInstance] selectRowsModelFromTable:WXBankCardTableName class:WXBankCard.class];
