...
+ (instancetype)sessionForUser:(WXUser *)user {
    ...
    if (session) {
        ...
        session.user = user;
        session.uid = user.uid;
        if ([[MNDatabase database] insertToTable:WXSessionTableName model:session] && [[MNDatabase database] createTable:session.list class:WXMessage.class]) {
            /// 发送添加会话通知
            [MNFileManager createDirectoryAtPath:[WechatHelper.helper.directoryPath stringByAppendingFormat:@"/%@", session.identifier] error:nil];
...
- (WXMessage *)message {
    if (!_message && _latest.length > 0 && _list.length > 0) {
        NSArray <WXMessage *>* msgs = [[MNDatabase database] selectRowsModelFromTable:_list where:@{@"identifier":_latest}.componentString limit:NSMakeRange(0, 1) class:[WXMessage class]];
        if (msgs.count > 0) {
