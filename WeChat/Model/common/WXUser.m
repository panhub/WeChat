...
- (void)updateUserInfo:(NSDictionary *)userInfo {
    ...
    _only = [NSDictionary boolValueWithDictionary:userInfo forKey:kPath(self.only)];
...
- (void)encodeWithCoder:(NSCoder *)coder {
    ...
    [coder encodeBool:self.only forKey:kPath(self.only)];
...
- (instancetype)initWithCoder:(NSCoder *)coder {
    if (self = [super init]) {
        ...
        self.only = [coder decodeBoolForKey:kPath(self.only)];
...
- (id)JsonValue {
    ...
    [dic setObject:@(self.only).stringValue forKey:kPath(self.only)];
    return dic.copy;
}
...
- (BOOL)isEqual:(id)object {
    if (object && [object isKindOfClass:WXUser.class]) {
        return [((WXUser *)object).uid isEqualToString:self.uid];
    }
    return NO;
}

- (BOOL)isEqualToUser:(WXUser *)user {
    if (user && [user isMemberOfClass:WXUser.class]) {
        return [user.uid isEqualToString:self.uid];
    }
    return NO;
}
