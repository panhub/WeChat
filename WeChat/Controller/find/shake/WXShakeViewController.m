...
- (void)createView {
    ...
    [titles enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        control.bottom_mn = self.contentView.height_mn - MAX(UITabSafeHeight() + 5.f, 10.f);
        control.title = obj;
...
- (void)showShakeMatchResult {
    ...
    if (self.type == WXShakeMatchPerson) {
        // 匹配人
        self.personCard.user = MNChatHelper.generateRandomUser;
        ...
        [UIView animateWithDuration:.6f animations:^{
            ...
            [MNDatabase insertIntoTable:WXShakeHistoryTableName model:history completion:nil];
        }];
...
- (void)personCardTouchUpInside:(WXShakePersonCard *)personCard {
    WXUserInfoViewController *vc = [[WXUserInfoViewController alloc] initWithUser:personCard.user];
    [self.navigationController pushViewController:vc animated:YES];
}
