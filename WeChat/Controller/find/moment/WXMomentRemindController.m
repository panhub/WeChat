...
- (void)loadData {
    dispatch_async_default(^{
        NSArray <WXMomentRemind *>*rows = [[MNDatabase sharedInstance] selectRowsModelFromTable:WXMomentRemindTableName class:WXMomentRemind.class];
        NSMutableArray <WXMomentRemindViewModel *>*dataArray = [NSMutableArray arrayWithCapacity:rows.count];
        [rows.reverseObjects enumerateObjectsUsingBlock:^(WXMomentRemind * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
