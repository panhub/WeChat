...
- (void)createView {
    ...
    videoCropView.bottom_mn = self.contentView.height_mn - MN_TAB_SAFE_HEIGHT - 30.f;
    ...
    CGFloat duration = [MNAssetExporter exportDurationWithMediaAtPath:self.videoPath];
    if (duration > 15.f) {
    ...
    UILabel *cutTimeLabel = [UILabel labelWithFrame:CGRectMake(0.f, 0.f, backgroundView.width_mn, 12.f) text:@"" alignment:NSTextAlignmentCenter textColor:quickLabel.textColor font:[UIFont systemFontOfSize:12.f]];
    cutTimeLabel.hidden = YES;
    cutTimeLabel.centerY_mn = (self.contentView.height_mn - videoCropView.bottom_mn - MN_TAB_SAFE_HEIGHT)/2.f + videoCropView.bottom_mn - backgroundView.top_mn;
    ...
    UILabel *timeLabel = [UILabel labelWithFrame:CGRectMake(0.f, 0.f, self.contentView.width_mn, quickLabel.font.pointSize) text:@"" alignment:NSTextAlignmentCenter textColor:quickLabel.textColor font:quickLabel.font];
    timeLabel.bottom_mn = backgroundView.top_mn - WXCropViewMargin;
    timeLabel.text = NSStringWithFormat(@"00:00/%@",[NSDate timeStringWithInterval:@(duration)]);
...
- (void)navigationBarRightBarItemTouchUpInside:(UIView *)rightBarItem {
    ...
    if (isFrameResizing) {
        ...
        exporter.timeRange = [exporter.asset timeRangeFromProgress:cutRange.location toProgress:MNMaxRange(cutRange)];
        ...
        session.timeRange = [session.asset timeRangeFromProgress:cutRange.location toProgress:MNMaxRange(cutRange)];
