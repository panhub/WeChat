        ...
        if (r.code == MNURLResponseCodeSucceed) {
            [ZMRegularRequest.new loadData:^{
                ...
                if (response.code == MNURLResponseCodeSucceed) {
                    ...
                    @strongify(self);
                    [self.contentView showWechatError:response.message];
                }
            }];
        } else {
            @strongify(self);
            [self.contentView showWechatError:r.message];
        }
    }];
}
...
- (void)extract {
    ...
    [[[ZMVideoUrlRequest alloc] initWithVideoUrl:string] loadData:^{
        ...
        if (response.code == MNURLResponseCodeSucceed) {
            ...
            [weakself.contentView showWechatError:response.message];
        }
    }];
}
...
- (void)download:(NSString *)url {
    if (url.length <= 0) {
        [self.contentView showWechatError:@"查询视频失败"];
        return;
    }
    @weakify(self);
