...
/**性别限制*/
@property (nonatomic) MNGender gender;
/**判断是否在刷新*/
...
- (void)loadData {
    ...
    dispatch_async_default(^{
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (self.gender != MNGenderUnknown) user.gender = self.gender;
...
#pragma mark - 数据更新
- (void)filterDataWithGender:(MNGender)gender {
    @weakify(self);
...
- (void)repayDataWithGender:(MNGender)gender {
    // 先挑出不合格数据
    NSArray *filterArray = gender == MNGenderUnknown ? @[] : [self.dataArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self.gender != %d", gender]];
    [self.dataArray removeObjectsInArray:filterArray];
    NSArray *cacheArray = gender == MNGenderUnknown ? self.dataCache.copy : [self.dataCache filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self.gender == %d", gender]];
...
- (void)navigationBarRightBarItemTouchUpInside:(UIView *)rightBarItem {
    ...
    [[MNActionSheet actionSheetWithTitle:nil cancelButtonTitle:@"取消" handler:^(MNActionSheet *actionSheet, NSInteger buttonIndex) {
        ...
        @strongify(self);
        self.gender = MNGenderFemale - buttonIndex;
    } otherButtonTitles:@"只看女生", @"只看男生", @"不限", nil] show];
}

#pragma mark - Setter
- (void)setGender:(MNGender)gender {
