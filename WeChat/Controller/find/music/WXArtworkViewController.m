...
- (void)createView {
    ...
    needleView.top_mn = self.contentInset.top;
    needleView.width_mn = 121.f;
    [needleView sizeFitToWidth];
    needleView.centerX_mn = scrollView.width_mn/2.f;
    ...
    UILabel *lyricLabel = [UILabel labelWithFrame:CGRectMake(0.f, 0.f, scrollView.width_mn - 30.f, 35.f) text:nil alignment:NSTextAlignmentCenter textColor:nil font:UIFontRegular(16.f)];
    lyricLabel.centerX_mn = scrollView.width_mn/2.f;
    lyricLabel.bottom_mn = CGRectGetMaxY(UIEdgeInsetsInsetRect(scrollView.bounds, self.contentInset));
    lyricLabel.numberOfLines = 1;
    lyricLabel.userInteractionEnabled = NO;
    [scrollView addSubview:lyricLabel];
    self.lyricLabel = lyricLabel;
    
    CGFloat wh = floor(MIN(lyricLabel.top_mn - needleView.bottom_mn + 50.f, scrollView.width_mn - 45.f));
    ...
    coverView.top_mn = needleView.bottom_mn - 50.f;
    ...
    // 缩略图
    wh = floor(308.f/476.f*discView.width_mn);
    ...
    lyricLabel.centerY_mn = (CGRectGetMaxY(UIEdgeInsetsInsetRect(scrollView.bounds, self.contentInset)) - coverView.bottom_mn)/2.f + coverView.bottom_mn;
    
    [scrollView bringSubviewToFront:needleView];
}
