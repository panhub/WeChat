...
#define WXMusicPlayerWhiteColor    R_G_B(248.f, 248.f, 255.f)
#define WXMusicPlayerNormalColor  R_G_B(145.f, 154.f, 165.f)
...
- (void)viewDidLoad {
    ...
    UILabel *durationLabel = [UILabel labelWithFrame:CGRectMake(0.f, 0.f, slider.left_mn - 7.f, 13.f) text:@"00:00" textAlignment:NSTextAlignmentRight textColor:WXMusicPlayerNormalColor font:[UIFont systemFontOfSize:13.f]];
    ...
    controlView.height_mn = playButton.bottom_sd + MAX(35.f, UITabSafeHeight() + 10.f);
...
- (void)loadData {
    ...
    self.player.playURLs = URLs.copy;
...
- (void)playerDidPlayTimeInterval:(MNPlayer *)player {
    ...
    self.durationLabel.text = [NSDate playTimeStringWithInterval:@(playTimeInterval)];
}
...
- (void)playerDidEndDecode:(MNPlayer *)player {
    ...
    self.totalLabel.text = [NSDate playTimeStringWithInterval:@(player.duration)];
}
...
#pragma mark - 更新背景
- (void)updateSubviews {
    self.navigationBar.titleColor = WXPreference.preference.playStyle == WXMusicPlayStyleDark ? WXMusicPlayerWhiteColor : WXMusicPlayerDarkColor;
    self.singerLabel.textColor = self.navigationBar.titleColor;
    self.darkBlurView.alpha = WXPreference.preference.playStyle == WXMusicPlayStyleDark;
    self.lightBlurView.alpha = WXPreference.preference.playStyle == WXMusicPlayStyleLight;
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    return WXPreference.preference.playStyle == WXMusicPlayStyleDark ? UIStatusBarStyleLightContent : UIStatusBarStyleDefault;
}
...
- (void)navigationBarDidCreateBarItem:(MNNavigationBar *)navigationBar {
    ...
    // 歌手
    UILabel *singerLabel = [UILabel labelWithFrame:CGRectMake(navigationBar.titleView.left_mn, 0.f, navigationBar.titleView.width_mn, 14.f) text:self.songs[self.playIndex].artist textAlignment:NSTextAlignmentCenter textColor:nil font:UIFontRegular(12.f)];
    //singerLabel.bottom_mn = navigationBar.height_mn - 1.f;
    [navigationBar addSubview:singerLabel];
    self.singerLabel = singerLabel;
    // 约束
    CGFloat margin = 3.f;
    CGFloat y = (navigationBar.height_mn - UIStatusBarHeight() - navigationBar.titleView.height_mn - singerLabel.height_mn - margin)/2.f + UIStatusBarHeight();
