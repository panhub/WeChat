...
// 未接通切换语音按钮
@property (nonatomic, strong) WXVoipButton *switchButton;
// 取消/挂断按钮
@property (nonatomic, strong) WXVoipButton *declineButton;
// 接通按钮
@property (nonatomic, strong) WXVoipButton *answerButton;
// 接通后切换语音按钮
@property (nonatomic, strong) WXVoipButton *voiceButton;
// 切换摄像头按钮
@property (nonatomic, strong) WXVoipButton *cameraButton;
// 背景视频播放
...
- (void)createView {
    ...
    backgroundPlayView.panGestureRecognizer.enabled = NO;
    ...
    // 提示信息
    UILabel *hintLabel = [UILabel labelWithFrame:CGRectZero text:(self.style == WXVoipStyleSend ? @"正在等待对方接受邀请" : @"邀请你视频通话") alignment:NSTextAlignmentRight textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:15.f]];
    ...
    // 切换语音视图
    WXVoipButton *switchButton = [[WXVoipButton alloc] initWithFrame:CGRectMake(0.f, 0.f, 42.f, 57.f)];
    switchButton.title = @"切到语音接听";
    switchButton.image = [UIImage imageNamed:@"call_voice_convert"];
    ...
    badgePlayView.panGestureRecognizer.enabled = NO;
...
#pragma mark - Event
- (void)buttonTouchUpInside:(WXVoipButton *)sender {
...
- (void)beginWaitingTimer {
    self.callIndex = -1;
    self.hintLabel.text = self.style == WXVoipStyleSend ? @"正在等待对方接受邀请" : @"邀请你视频通话";
    @weakify(self);
