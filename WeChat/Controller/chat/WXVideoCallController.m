...
- (void)createView {
    ...
    UILabel *backgroundLabel = [UILabel labelWithFrame:backgroundImageView.bounds text:@"点击添加视频或图片" textAlignment:NSTextAlignmentCenter textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:17.f]];
    ...
    /// 对方账户昵称
    UILabel *nickLabel = [UILabel labelWithFrame:CGRectZero text:self.user.nickname textAlignment:NSTextAlignmentRight textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:29.f]];
    [nickLabel sizeToFit];
    ...
    // 提示信息
    UILabel *hintLabel = [UILabel labelWithFrame:CGRectZero text:(self.style == WXVideoCallSend ? @"正在等待对方接受邀请" : @"邀请你视频通话") textAlignment:NSTextAlignmentRight textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:15.f]];
    ...
    [imgs enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        button.bottom_mn = self.contentView.height_mn - MAX(25.f, (UITabSafeHeight() + 5.f));
        if (self.style == WXVideoCallSend) {
    ...
    badgeImageView.top_mn = UIStatusBarHeight();
    ...
    UILabel *badgeLabel = [UILabel labelWithFrame:badgeImageView.bounds text:@"点击添加视频或图片" textAlignment:NSTextAlignmentCenter textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:15.f]];
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        if (asset.type == MNAssetTypeVideo) {
            if (view == self.backgroundPlayView || view == self.backgroundImageView) {
                ...
                self.backgroundPlayer.playURLs = @[[NSURL fileURLWithPath:asset.content]];
                [self.backgroundPlayer play];
            } else {
                self.badgePlayView.alpha = 1.f;
                self.badgeImageView.alpha = 0.f;
                self.badgeImageView.image = asset.thumbnail;
                self.badgePlayer.playURLs = @[[NSURL fileURLWithPath:asset.content]];
