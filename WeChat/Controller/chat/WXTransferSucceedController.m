...
- (void)createView {
    ...
    UIImageView *succeedView = [UIImageView imageViewWithFrame:CGRectMake(MEAN(self.contentView.width_mn - size.width), TOP_BAR_HEIGHT + 5.f, size.width, size.height) image:image];
    [self.contentView addSubview:succeedView];
    /// 转账成功文字
    UILabel *succeedLabel = [UILabel labelWithFrame:CGRectMake(0.f, succeedView.bottom_mn + 15.f, self.contentView.width_mn, 17.f) text:@"支付成功" textAlignment:NSTextAlignmentCenter textColor:R_G_B(26.f, 173.f, 25.f) font:UIFontRegular(17.f)];
    [self.contentView addSubview:succeedLabel];
    
    /// 收款
    UILabel *nameLabel = [UILabel labelWithFrame:CGRectMake(0.f, 200.f, self.contentView.width_mn, 20.f) text:[NSString stringWithFormat:@"待%@确认收款", self.user.name] textAlignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor darkTextColor], .85f) font:[UIFont systemFontOfSize:17.f]];
...
- (void)buttonClicked {
    [self.navigationController popToViewController:[self.navigationController findViewControllerOfClass:NSClassFromString(@"WXChatViewController")] animated:YES];
}
