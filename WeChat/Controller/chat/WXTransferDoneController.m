...
- (void)createView {
    ...
    UIImageView *doneView = [UIImageView imageViewWithFrame:CGRectMake(0.f, TOP_BAR_HEIGHT + 52.f, 56.f, 56.f) image:UIImageNamed(@"wx_transfer_success")];
    ...
    UILabel *doneLabel = [UILabel labelWithFrame:CGRectMake(0.f, doneView.bottom_mn + 40.f, self.contentView.width_mn, 16.f) text:text textAlignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor darkTextColor], .9f) font:UIFontRegular(16.f)];
    ...
    /// 收款时间
    NSString *time = [NSDate dateStringWithTimestamp:self.redpacket.draw_time format:@"yyyy-MM-dd HH:mm:ss"];
    UILabel *timeLabel = [UILabel labelWithFrame:CGRectMake(0.f, self.contentView.height_mn - 75.f, self.contentView.width_mn, 12.f) text:[@"收款时间: " stringByAppendingString:time] textAlignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor darkTextColor], .5f) font:[UIFont systemFontOfSize:12.f]];
    [self.contentView addSubview:timeLabel];
    
    /// 转账时间
    time = [NSDate dateStringWithTimestamp:self.redpacket.create_time format:@"yyyy-MM-dd HH:mm:ss"];
    timeLabel = [UILabel labelWithFrame:CGRectMake(0.f, self.contentView.height_mn - 97.f, self.contentView.width_mn, 12.f) text:[@"转账时间: " stringByAppendingString:time] textAlignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor darkTextColor], .5f) font:[UIFont systemFontOfSize:12.f]];
    [self.contentView addSubview:timeLabel];
...
- (void)navigationBarLeftBarItemTouchUpInside:(UIView *)leftBarItem {
    if (self.cls.length) {
        [self.navigationController popToViewController:[self.navigationController findViewControllerOfClass:NSClassFromString(self.cls)] animated:YES];
    } else {
