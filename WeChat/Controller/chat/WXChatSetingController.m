...
- (void)loadData {
    ...
    model0.title = @"置顶聊天";
    model0.value = NSStringFromNumber(@(self.session.front));
    model0.desc = @"";
    WXDataValueModel *model1 = [WXDataValueModel new];
    model1.title = @"消息免打扰";
    model1.value = NSStringFromNumber(@(self.session.remind));
    model1.desc = @"";
    WXDataValueModel *model2 = [WXDataValueModel new];
    model2.title = @"消息未读数";
    model2.value = @"0";
    model2.desc = NSStringFromNumber(@(self.session.unread_count));
...
- (void)headerViewAvatarButtonTouchUpInside:(WXChatSettingHeaderView *)headerView {
    WXUserInfoViewController *vc = [[WXUserInfoViewController alloc] initWithUser:self.session.user];
    [self.navigationController pushViewController:vc animated:YES];
}
