...
- (void)createView {
    ...
    //昵称
    UILabel *nickLabel = [UILabel labelWithFrame:CGRectZero text:self.user.name alignment:NSTextAlignmentCenter textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:29.f]];
    ...
    // 提示信息
    UILabel *hintLabel = [UILabel labelWithFrame:CGRectZero text:(self.style == WXVoiceCallSend ? @"正在等待对方接受邀请" : @"邀请你语音通话") alignment:NSTextAlignmentCenter textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:15.f]];
    ...
    [imgs enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        button.bottom_mn = self.contentView.height_mn - MAX(25.f, (MN_TAB_SAFE_HEIGHT + 8.f));
        if (self.style == WXVoiceCallSend) {
