...
// 是否接通
@property (nonatomic) WXVoipState state;
// 标记是否可手势返回
...
// 取消/挂断按钮
@property (nonatomic, strong) WXVoipButton *declineButton;
// 接通按钮
@property (nonatomic, strong) WXVoipButton *answerButton;
// 静音按钮
@property (nonatomic, strong) WXVoipButton *muteButton;
// 免提按钮
@property (nonatomic, strong) WXVoipButton *speakerButton;
@end
...
- (void)createView {
    ...
    // 头像
    UIButton *avatarButton = [UIButton buttonWithFrame:CGRectMake(0.f, 0.f, 128.f, 128.f) image:self.user.avatar title:nil titleColor:nil titleFont:nil];
    avatarButton.centerX_mn = self.contentView.width_mn/2.f;
    //avatarButton.bottom_mn = 975.f/2208.f*self.contentView.height_mn;
    avatarButton.top_mn = ceil((self.contentView.height_mn - avatarButton.height_mn)/3.f);
    avatarButton.layer.cornerRadius = 5.f;
    avatarButton.clipsToBounds = YES;
    [self.contentView addSubview:avatarButton];
    
    //昵称
    UILabel *nickLabel = [UILabel labelWithFrame:CGRectZero text:self.user.name alignment:NSTextAlignmentCenter textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:30.f]];
    nickLabel.numberOfLines = 1;
    [nickLabel sizeToFit];
    nickLabel.top_mn = avatarButton.bottom_mn + 15.f;
    nickLabel.centerX_mn = self.contentView.width_mn/2.f;
    [self.contentView addSubview:nickLabel];
    
    // 提示信息
    UILabel *hintLabel = [UILabel labelWithFrame:CGRectZero text:(self.style == WXVoipStyleSend ? @"正在等待对方接受邀请" : @"邀请你语音通话") alignment:NSTextAlignmentCenter textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:15.f]];
    ...
    CGFloat wh = 71.f; // 间隔
    CGFloat x = floor((self.contentView.width_mn - wh*3.f)/4.f); // 起始坐标
    [imgs enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        WXVoipButton *button = [[WXVoipButton alloc] init];
        button.size_mn = CGSizeMake(wh, wh + 28.f);
        button.bottom_mn = self.contentView.height_mn - 25.f - MN_TAB_SAFE_HEIGHT;
        if (self.style == WXVoipStyleSend) {
    ...
    [imgs enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSInteger tag = tags[idx].integerValue;
        WXVoipButton *button = [[WXVoipButton alloc] initWithFrame:self.declineButton.frame];
        button.left_mn = idx == 0 ? x : (self.contentView.width_mn - wh - x);
        button.title = titles[idx];
        button.image = [UIImage imageNamed:obj];
    ...
    UILabel *durationLabel = [UILabel labelWithFrame:CGRectZero text:@"00:01" alignment:NSTextAlignmentCenter textColor:UIColorWithAlpha(UIColor.whiteColor, .95f) font:[UIFont systemFontOfSize:17.f]];
...
#pragma mark - Event
// 按钮点击
- (void)buttonTouchUpInside:(WXVoipButton *)button {
...
- (void)declineCalling {
    ...
    [self endAnswerTimer];
    if (self.state == WXVoipStateWaiting) self.state = WXVoipStateDecline;
...
- (void)updateCallDuration {
    self.durationLabel.text = [NSDate timeStringWithInterval:@(self.duration)];
...
- (void)beginWaitingTimer {
    self.callIndex = -1;
    self.hintLabel.text = self.style == WXVoipStyleSend ? @"正在等待对方接受邀请" : @"邀请你语音通话";
    @weakify(self);
    dispatch_timer_main(WXVoiceCallWaitingTimerName, .7f, ^{
        ...
        if (self.callIndex >= 3) {
            self.callIndex = -1;
            if (self.style == WXVoipStyleSend) self.waitIndex ++;
        }
        self.hintLabel.text = text;
        if (self.waitIndex >= 20) {
            self.state = WXVoipStateRefuse;
            [self declineCalling];
        }
    });
}
...
- (NSString *)description {
    if (self.style == WXVoipStyleSend) {
        if (self.state == WXVoipStateDecline) {
            return @"已取消";
        } else if (self.state == WXVoipStateRefuse) {
            return @"对方已拒绝";
        }
...
- (MNTransitionAnimator *)popTransitionAnimator {
    WXCallTransitionAnimator *animator = WXCallTransitionAnimator.new;
    animator.decline = self.state == WXVoipStateDecline || self.state == WXVoipStateRefuse;
