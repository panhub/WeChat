...
- (void)loadData {
    ...
    model1.value = @"0";
    model1.desc = @" ";
    if (self.user.gender == MNGenderMale) {
        model1.title = @"把他推荐给朋友";
    } else if (self.user.gender == MNGenderFemale) {
...
- (void)sendCardToUser:(WXUser *)toUser msg:(NSString *)text inViewController:(UIViewController *)vc {
    ...
    [vc.view showWeChatDialogDelay:.35f eventHandler:^{
        ...
        if (![MNDatabase.sharedInstance updateTable:WXSessionTableName where:@{sql_field(session.identifier):session.identifier}.componentString model:session]) return;
        ...
        if (isSucceed) {
            ...
            if (!session) return;
            // 检查会话里是否有消息, 没有就删除
            if ([MNDatabase.sharedInstance selectRowsModelFromTable:session.list class:WXMessage.class].count <= 0) {
...
- (void)deleteUserButtonClicked {
    ...
    MNActionSheet *actionSheet = [MNActionSheet actionSheetWithTitle:@"同时删除与该联系人的聊天记录" cancelButtonTitle:@"取消" handler:^(MNActionSheet *sheet, NSInteger buttonIndex) {
        ...
        [self.view showWeChatDialogDelay:.3f eventHandler:^{
            ...
            if ([self.navigationController findViewControllerOfClass:NSClassFromString(@"WXChatViewController")]) {
