...
- (void)createView {
    ...
    UIImageView *shadowView = [[UIImageView alloc] initWithImage:UIImageNamed(@"wx_moment_comment_horizontal_line")];
...
- (void)loadData {
    ...
    /// 获取通讯录
    NSArray <WXUser *>*contacts = [[[MNChatHelper helper] contacts] copy];
    if ([contacts count] > 0) {
    ...
    /// 接受通讯录重载回调
    [MNChatHelper helper].contactsUpdateHandler = ^(NSArray<WXUser *> *contacts) {
...
- (void)viewDidLoad {
    [super viewDidLoad];
    // 检索结果展示
    WXContactsResultController *searchResultController = [[WXContactsResultController alloc] initWithFrame:CGRectMake(0.f, self.navigationBar.height_mn + self.tableView.tableHeaderView.height_mn, self.view.width_mn, self.view.height_mn - UIStatusBarHeight() - self.searchBar.height_mn)];
...
- (void)navigationBarRightBarItemTouchUpInside:(UIView *)rightBarItem {
    if ([[[MNChatHelper helper] contacts] count] >= 300) {
        [self.view showInfoDialog:@"已达到最大联系人数量"];
        return;
    }
    @weakify(self);
    [[MNActionSheet actionSheetWithTitle:@"添加用户" cancelButtonTitle:@"取消" handler:^(MNActionSheet *actionSheet, NSInteger buttonIndex) {
        if (buttonIndex == actionSheet.cancelButtonIndex) return;
        if (buttonIndex == 0) {
            /// 自动添加
            [[MNActionSheet actionSheetWithTitle:@"联系人数量" cancelButtonTitle:@"取消" handler:^(MNActionSheet *sheet, NSInteger index) {
                ...
                [[MNChatHelper helper] createContacts:[count unsignedIntegerValue] completion:^(NSArray<WXUser *> *contacts) {
                    if (contacts.count > 0) {
...
- (void)reloadList {
    ...
    self.footerLabel.text = NSStringWithFormat(@"%@位联系人", @([[[MNChatHelper helper] contacts] count]));
}
...
- (UIImage *)tabBarItemImage {
    return UIImageNamed(@"wx_tabbar_contacts");
}

- (UIImage *)tabBarItemSelectedImage {
    return UIImageNamed(@"wx_tabbar_contactsHL");
}
