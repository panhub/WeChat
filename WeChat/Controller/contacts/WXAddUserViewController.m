...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 6) {
        ...
        [ac setButtonTitleColor:R_G_B(70.f, 182.f, 239.f) ofIndex:0];
        [ac setButtonTitleColor:R_G_B(243.f, 126.f, 125.f) ofIndex:1];
...
- (void)createContacts {
    ...
    if (self.headerView.headButton.selected) {
        ...
        [user setValue:[MNChatHelper randomAvatarImage] forKey:@"avatar"];
    }
    user.avatarData = user.avatar.PNGData;
    if ([[MNChatHelper helper] insertUserToContacts:user]) {
