...
#pragma mark - 更新用户数据
- (void)loadData {
    self.containsUser = ([WechatHelper.helper containsUser:self.user] && ![self.user.uid isEqualToString:WXUser.shareInfo.uid]);
    ...
    if (indexPath.section == 0) {
        ...
        if (!self.isContainsUser) {
            ...
            [self.view showWechatDialogDelay:.35f eventHandler:^{
                isSucceed = [[WechatHelper helper] insertUserToContacts:self.user];
            } completionHandler:^{
                if (isSucceed) {
                    [self loadData];
                } else {
