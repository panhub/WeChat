...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    WXSessionListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"com.wx.home.result.cell"];
    if (!cell) {
        cell = [[WXSessionListCell alloc] initWithReuseIdentifier:@"com.wx.home.result.cell" size:CGSizeMake(tableView.width_mn, tableView.rowHeight)];
    }
...
- (void)updateSearchResultText:(NSString *)text forSearchController:(MNSearchViewController *)searchController {
    ...
    if (text.length > 0) {
        [[[MNChatHelper helper] sessions] enumerateObjectsUsingBlock:^(WXSession * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
