...
- (void)viewDidLoad {
    [super viewDidLoad];
    /// 检索结果展示
    WXSessionResultController *searchResultController = [[WXSessionResultController alloc] initWithFrame:CGRectMake(0.f, self.navigationBar.height_mn + self.tableView.tableHeaderView.height_mn, self.view.width_mn, self.view.height_mn - UIStatusBarHeight() - self.searchBar.height_mn)];
...
- (void)loadData {
    [self.dataArray addObjectsFromArray:[[MNChatHelper helper] sessions]];
    /// 接受会话列表重载回调
    [MNChatHelper helper].sessionUpdateHandler = ^(NSArray<WXSession *> *sessions) {
