...
- (void)createView {
    ...
    scanView.cornerSize = CGSizeMake(2.5f, 23.f);
...
- (void)scannerDidChangeTorchScene:(MNScanner *)scanner {
    BOOL isTorchScene = scanner.isTorchScene;
    self.lightButton.selected = isTorchScene;
    self.lightLabel.text = isTorchScene ? @"轻触关闭" : @"轻触点亮";
    self.lightLabel.hidden = self.lightButton.hidden = !isTorchScene;
}

- (void)scannerUpdateSampleBrightness:(CGFloat)brightnessValue {
    if (self.scanner.isTorchScene) return;
