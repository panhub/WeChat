...
- (void)createView {
    ...
    if ([[[WechatHelper helper] cards] count] > 0) cardView.card = [[[WechatHelper helper] cards] firstObject];
    ...
    [accessButton setBackgroundImage:[UIImage imageWithColor:MN_R_G_B(156.f, 229.f, 191.f)] forState:UIControlStateDisabled];
...
- (void)accessButtonClicked {
    ...
    if (self.type == WXChangeAccessRecharge) {
        ...
        /// 更新银行卡
        card.money -= money;
        if ([[WechatHelper helper] updateBankCard:card]) {
            /// 插入零钱记录
