...
- (void)loadData {
    ...
    NSString *gender = @"";
    if (user.gender == MNGenderUnknown) {
        gender = @"保密";
    } else {
        gender = user.gender == MNGenderMale ? @"男" : @"女";
    }
    NSArray <NSString *>*titles = @[@"性别", @"地区", @"个性签名"];
    NSArray <NSString *>*descs = @[gender , [NSString replacingBlankCharacter:user.location withCharacter:@""], [NSString replacingBlankCharacter:user.signature withCharacter:@""]];
    ...
    if (row == 0) {
        ...
        [[MNActionSheet actionSheetWithTitle:@"选择性别" cancelButtonTitle:@"取消" handler:^(MNActionSheet *actionSheet, NSInteger buttonIndex) {
            ...
            [WXUser performReplacingHandler:^(WXUser *userInfo) {
                if (buttonIndex == 2) {
                    userInfo.gender = MNGenderUnknown;
                } else {
                    userInfo.gender = buttonIndex ? MNGenderFemale : MNGenderMale;
                }
            }];
