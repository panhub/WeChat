...
- (void)createView {
    ...
    if ([WXUser shareInfo].gender == MNGenderUnknown) {
        genderView.width_mn = 0.f;
    } else {
        genderView.width_mn = genderView.height_mn;
        genderView.image = [UIImage imageNamed:([WXUser shareInfo].gender == MNGenderMale ? @"wx_contacts_gender_male" : @"wx_contacts_gender_female")];
    }
...
- (void)loadData {
    ...
    [dic setObject:[NSString replacingBlankCharacter:[[WXUser shareInfo] uid]] forKey:@"uid"];
    [dic setObject:[NSString replacingBlankCharacter:[[WXUser shareInfo] number]] forKey:@"number"];
    [dic setObject:[NSString replacingBlankCharacter:[[WXUser shareInfo] location]] forKey:@"location"];
    [dic setObject:[NSString replacingBlankCharacter:[[WXUser shareInfo] wechatId]] forKey:@"wechatId"];
    [dic setObject:[NSString replacingBlankCharacter:[[WXUser shareInfo] nickname]] forKey:@"nickname"];
...
- (void)handTap:(UIGestureRecognizer *)recognizer {
    ...
    [browser presentInView:self.view fromAsset:asset animated:YES completion:nil];
}
...
- (void)createQRCodeWithColor:(UIColor *)color {
    [MNQRCode createQRCodeWithMetadata:[self.dataSource.JsonString stringByInsertString:WXQRCodeMetadataPrefix atIndex:0].JsonData color:color pixel:400.f completion:^(UIImage *image) {
