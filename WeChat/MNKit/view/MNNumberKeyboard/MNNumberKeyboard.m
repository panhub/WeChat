...
- (void)createView {
    ...
    [self gridLayoutWithInitial:CGRectMake(0.f, 0.f, interval, MNNumberKeyButtonHeight) offset:UIOffsetZero count:titles.count handler:^(CGRect rect, NSUInteger idx, BOOL *stop) {
        ...
        if (idx == 11) {
            ...
            button.imageEdgeInsets = UIEdgeInsetsMake((button.height_mn - size.height)/2.f, (button.width_mn - size.width)/2.f, (button.height_mn - size.height)/2.f, (button.width_mn - size.width)/2.f);
        }
        [button addTarget:self action:@selector(buttonClicked:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:button];
        height = button.bottom_mn;
    }];
    self.height_mn = height + MN_TAB_SAFE_HEIGHT;
