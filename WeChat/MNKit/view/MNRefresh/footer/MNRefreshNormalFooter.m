...
- (MNIndicatorView *)indicatorView {
    if (!_indicatorView) {
        MNIndicatorView *indicatorView = [[MNIndicatorView alloc] initWithFrame:CGRectMake(0.f, 0.f, 16.f, 16.f)];
...
- (UILabel *)noDataLabel {
    if (!_noDataLabel) {
        ...
        noDataLabel.textColor = [UIColor.darkGrayColor colorWithAlphaComponent:.7f];
...
- (void)setState:(MJRefreshState)state
{
    ...
    self.textLabel.hidden = self.indicatorView.hidden = !self.noDataLabel.hidden;
    if (state == MJRefreshStateRefreshing) [self.indicatorView startAnimating];
}
