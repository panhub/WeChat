...
static NSString *MNPageMinContentSizeKey = @"com.mn.page.min.content.size.key";
...
- (BOOL)contentSizeReached {
    ...
    if (reachContentSize) return [reachContentSize boolValue];
    return NO;
}
...
- (BOOL)observed {
    ...
    if (observed) return [observed boolValue];
    return NO;
}

- (void)setGuessMinContentSize:(CGSize)guessMinContentSize {
    objc_setAssociatedObject(self, &MNPageMinContentSizeKey, [NSValue valueWithCGSize:guessMinContentSize], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CGSize)guessMinContentSize {
    NSValue *value = objc_getAssociatedObject(self, &MNPageMinContentSizeKey);
