...
- (void)reloadSegment {
    NSInteger selectedIndex = _segmentView.selectedIndex;
    [_segmentView reloadRightView];
    [_segmentView reloadTitles];
    [_segmentView updateSelectIndex:selectedIndex];
    [_segmentView reloadData];
}
...
- (CGFloat)pageHeaderMaxOffsetY {
    return _headerView.bottom_mn - _fixedHeight;
}
...
- (void)pageDidScrollWithOffsetY:(CGFloat)offsetY ofIndex:(NSInteger)pageIndex {
     ...
     */
    CGFloat y = [self headerOriginYForOffsetY:(offsetY + [self pageInsetOfInitialized])];
    if (fabs(_profileView.top_mn - y) >= .01f) _profileView.top_mn = y;
    _contentOffset.y = offsetY;
    if ([_delegate respondsToSelector:@selector(segmentControllerPageDidScroll:)]) {
        [_delegate segmentControllerPageDidScroll:self];
    }
}
...
- (void)layoutPageOffsetY:(UIViewController <MNSegmentSubpageDataSource>*)page {
    ...
    CGFloat y = [self headerOriginYForOffsetY:(scrollView.contentOffset.y + [self pageInsetOfInitialized])];
...
- (CGFloat)headerOriginYForOffsetY:(CGFloat)offsetY {
    //上滑为正, 下滑为负
...
- (void)makeSeparatorVisible:(BOOL)isVisible {
    [_segmentView makeSeparatorVisible:isVisible];
}

#pragma mark - Setter
- (void)setFixedHeight:(CGFloat)fixedHeight {
    fixedHeight = MAX(0.f, fixedHeight);
    if (_headerView) fixedHeight = MIN(_headerView.bottom_mn, fixedHeight);
...
- (BOOL)isScrollEnabled {
    return _pageController.isScrollEnabled;
}

- (BOOL)isSelectEnabled {
    return _segmentView.isSelectEnabled;
}
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        _headerView = headerView ? : [[UIView alloc] initWithFrame:CGRectZero];
        _fixedHeight = MIN(_headerView.bottom_mn, _fixedHeight);
    }
    return _headerView;
}
...
- (UIViewController <MNSegmentSubpageDataSource>*)currentPageController {
    return [_pageController pageCacheOfIndex:_segmentView.selectedIndex];
}
...
- (NSInteger)currentPageOfIndex {
    if (_segmentView) return _segmentView.selectedIndex;
    return 0;
}
