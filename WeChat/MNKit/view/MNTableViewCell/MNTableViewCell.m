...
- (void)layoutSubviews {
    ...
    if (_separator && _separator.superview) _separator.bottom_mn = self.contentView.height_mn;
}
...
- (void)setSeparatorInset:(UIEdgeInsets)separatorInset {
    ...
    if (_separator && _separator.superview) {
        _separator.left_mn = separatorInset.left;
        _separator.width_mn = self.contentView.width_mn - separatorInset.left - separatorInset.right;
    }
}

- (void)setSeparatorColor:(UIColor *)separatorColor {
    _separatorColor = _separatorColor.copy;
    if (_separator && _separator.superview) {
        _separator.backgroundColor = _separatorColor;
    }
}

- (void)setSeparatorStyle:(MNTableViewCellSeparatorStyle)separatorStyle {
    _separatorStyle = separatorStyle;
    if (separatorStyle == MNTableViewCellSeparatorStyleNone) {
        if (_separator && _separator.superview) [_separator removeFromSuperview];
    } else {
        self.separator.backgroundColor = _separatorColor ? : [UIColor.darkGrayColor colorWithAlphaComponent:.1f];
        self.separator.left_mn = self.separatorInset.left;
        self.separator.width_mn = self.contentView.width_mn - self.separatorInset.left - self.separatorInset.right;
    }
}
...
- (UIView *)separator {
    if (!_separator) {
        ...
        separator.left_mn = self.separatorInset.left;
        separator.width_mn = self.contentView.width_mn - self.separatorInset.left - self.separatorInset.right;
        separator.height_mn = UIScreen.mainScreen.scale >= 3.f ? .7f : 1.f;
        separator.backgroundColor = _separatorColor ? : [UIColor.darkGrayColor colorWithAlphaComponent:1.f];
        [self.contentView addSubview:_separator = separator];
    }
    return _separator;
}
