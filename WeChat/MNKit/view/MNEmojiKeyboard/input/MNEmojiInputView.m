...
- (instancetype)initWithFrame:(CGRect)frame configuration:(MNEmojiKeyboardConfiguration *)configuration {
...
- (MNEmojiView *)emojiViewOfIndex:(NSUInteger)index {
    ...
    if (!emojiView) {
        ...
        emojiView.margin = self.contentInset.bottom;
        emojiView.emojiDelegate = self;
        emojiView.configuration = self.configuration;
        NSArray <MNEmojiPacket *>*emojiPackets = self.emojiPackets;
        if (self.currentPageIndex < emojiPackets.count) emojiView.packet = emojiPackets[index];
