...
- (void)updateElementView {
    if (!self.packet || self.packet.type == MNEmojiPacketTypeImage) {
        ...
        self.elementView.bottom_mn = self.height_mn - MAX(MN_TAB_SAFE_HEIGHT, inset/2.f);
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(nonnull MNEmojiCell *)cell forItemAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    cell.emoji = emoji;
    if (emoji.type == MNEmojiPacketTypeText) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [visibleCells enumerateObjectsUsingBlock:^(UICollectionViewCell * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect frame = [obj.superview convertRect:obj.frame toView:weakself];
        if (CGRectIntersectsRect(frame, weakself.elementView.frame)) {
            if (obj.alpha != 0.f) {
...
- (void)setPacket:(MNEmojiPacket *)packet {
    ...
    if (self.configuration.style == MNEmojiKeyboardStyleLight) {
        ...
        if (packet.type == MNEmojiPacketTypeImage) {
            layout.sectionInset = UIEdgeInsetsMake(inset, inset, MAX(MN_TAB_SAFE_HEIGHT, inset), inset);
        } else {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (weakself.window && !weakself.collectionView.isDragging && !weakself.collectionView.isDecelerating) {
        ...
        if (packet.type == MNEmojiPacketTypeText) {
            ...
            CGFloat ym = (self.height_mn - self.margin - itemSize.height*rows)/(rows + 1);
            CGFloat x = xm;
            CGFloat y = ym;
...
#pragma mark - Getter
- (CGSize)itemSize {
    if (self.packet.type == MNEmojiPacketTypeText) {
        return self.width_mn >= 390.f ? CGSizeMake(32.f, 32.f) : CGSizeMake(30.f, 30.f);
    }
    return self.width_mn >= 390.f ? CGSizeMake(70.f, 70.f) : CGSizeMake(63.f, 63.f);
}
...
- (CGFloat)insetOfEmojisInPage {
    CGSize itemSize = self.itemSize;
    NSInteger columns = self.numberOfColumnsInPage;
    CGFloat inset = (self.width_mn - itemSize.width*columns)/(columns + 1);
    return floor(inset);
}
