...
#endif
#if __has_include(<ImageIO/CGImageDestination.h>)
    ...
    if (model.type == MNAssetTypeVideo) {
        ...
        model.downloadId = [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
            ...
            if (model.type == MNAssetTypePhoto) {
                if (configuration && !configuration.isAllowsExportHEIF && model.asset.isHEIF) {
#ifdef __IPHONE_10_0
                ...
                if (configuration && !configuration.isOriginalExporting && configuration.isAllowsOptimizeExporting) {
...
+ (void)requestLivePhotoWithResourceFileURLs:(NSArray <NSURL *>*)fileURLs completion:(void(^_Nullable)(PHLivePhoto * _Nullable livePhoto, NSError *_Nullable error))completionHandler {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        __block NSURL *movURL = nil;
        __block NSURL *jpegURL = nil;
        [fileURLs enumerateObjectsUsingBlock:^(NSURL * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:NSURL.class] && obj.isFileURL && [NSFileManager.defaultManager fileExistsAtPath:obj.path]) {
                NSString *extension = obj.path.pathExtension.lowercaseString;
        ...
        if (!movURL || !jpegURL) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (completionHandler) completionHandler(nil, [NSError errorWithDomain:NSCocoaErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey:@"文件地址错误"}]);
            });
            return;
        }
        [PHLivePhoto requestLivePhotoWithResourceFileURLs:fileURLs placeholderImage:nil targetSize:CGSizeZero contentMode:PHImageContentModeAspectFit resultHandler:^(PHLivePhoto * _Nullable livePhoto, NSDictionary * _Nonnull info) {
            if (livePhoto) {
                if ([[info objectForKey:@"PHLivePhotoInfoIsDegradedKey"] boolValue]) return;
                NSURL *videoURL = [livePhoto valueForKey:@"videoURL"];
                if (!videoURL) [livePhoto setValue:movURL forKey:@"videoURL"];
                NSURL *imageURL = [livePhoto valueForKey:@"imageURL"];
                if (!imageURL) [livePhoto setValue:jpegURL forKey:@"imageURL"];
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (completionHandler) completionHandler(livePhoto, nil);
                });
            } else {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (completionHandler) completionHandler(nil, [NSError errorWithDomain:NSCocoaErrorDomain code:0 userInfo:@{NSLocalizedDescriptionKey:@"合成LivePhoto失败"}]);
                });
            }
        }];
    });
