...
- (void)setSelectIndex:(NSInteger)selectIndex {
    ...
    NSMutableArray *indexPaths = @[[NSIndexPath indexPathForItem:lastSelectIndex inSection:0]].mutableCopy;
    if (lastSelectIndex != selectIndex) [indexPaths addObject:[NSIndexPath indexPathForItem:selectIndex inSection:0]];
...
- (void)updateBottomMarginIfNeeded {
    ...
    if (asset.type == MNAssetTypeVideo) {
        ...
        [UIView animateWithDuration:.3f delay:0.f options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseInOut animations:^{
            ...
            self.bottom_mn = self.superview.height_mn - MN_TAB_SAFE_HEIGHT - MNAssetSelectBottomMaxMargin;
        } completion:nil];
    } else {
        if (self.bottom_mn == self.superview.height_mn) return;
        [UIView animateWithDuration:.3f delay:0.f options:UIViewAnimationOptionBeginFromCurrentState|UIViewAnimationOptionCurveEaseInOut animations:^{
            self.height_mn = self.collectionView.height_mn + MN_TAB_SAFE_HEIGHT + MNAssetSelectBottomMinMargin;
