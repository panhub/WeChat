...
@interface MNCaptureToolBar () <CAAnimationDelegate, UIGestureRecognizerDelegate>
...
const NSTimeInterval MNCaptureToolBarAnimationDuration = .3f;
...
- (void)didMoveToSuperview {
    ...
    if (self.options & MNCaptureOptionPhoto) {
        ...
        [tapGestureRecognizer addTarget:self action:@selector(handTap:)];
        [self.trackView addGestureRecognizer:tapGestureRecognizer];
    }
    if (self.options & MNCaptureOptionVideo) {
        ...
        longPressRecognizer.minimumPressDuration = .3f;
        [longPressRecognizer addTarget:self action:@selector(handLongPress:)];
        [self.trackView addGestureRecognizer:longPressRecognizer];
    }
}

#pragma mark - Gesture
- (void)handTap:(UITapGestureRecognizer *)recognizer {
    if (self.state == MNCaptureToolStateIdle && [self.delegate respondsToSelector:@selector(captureToolBarDidBeginTakingPhoto:)]) {
        [self.delegate captureToolBarDidBeginTakingPhoto:self];
    }
}
...
- (void)beginCapturing {
    ...
    if ([self.delegate respondsToSelector:@selector(captureToolBarDidBeginCapturingVideo:)]) {
        [self.delegate captureToolBarDidBeginCapturingVideo:self];
    }
}
...
- (void)endCapturing {
    ...
    if ([self.delegate respondsToSelector:@selector(captureToolBarDidEndCapturingVideo:)]) {
        [self.delegate captureToolBarDidEndCapturingVideo:self];
    }
}
...
- (void)startCapturing {
    ...
    [UIView animateWithDuration:MNCaptureToolBarAnimationDuration animations:^{
        ...
        if (self.timeoutInterval > 0.f) [self.progressLayer addAnimation:self.strokeAnimation forKey:@"com.mn.capturing.progress"];
...
- (void)stopCapturing {
    ...
    [UIView animateWithDuration:MNCaptureToolBarAnimationDuration animations:^{
        ...
        [UIView animateWithDuration:MNCaptureToolBarAnimationDuration delay:0.f options:UIViewAnimationOptionCurveEaseInOut animations:^{
            self.closeButton.alpha = 0.f;
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if (touch.view != self.trackView) return NO;
    if ([gestureRecognizer isKindOfClass:UILongPressGestureRecognizer.class]) {
        if ([self.delegate respondsToSelector:@selector(captureToolBarShouldCapturingVideo:)]) {
            BOOL s = [self.delegate captureToolBarShouldCapturingVideo:self];
            return s;//[self.delegate captureToolBarShouldCapturingVideo:self];
        }
    } else if ([gestureRecognizer isKindOfClass:UITapGestureRecognizer.class]) {
        if ([self.delegate respondsToSelector:@selector(captureToolBarShouldTakingPhoto:)]) {
            return [self.delegate captureToolBarShouldTakingPhoto:self];
        }
    }
    return YES;
}
