...
- (void)setAsset:(MNAsset *)asset {
    ...
    self.playToolBar.hidden = self.playView.hidden = asset.type != MNAssetTypeVideo;
    ...
    [[MNAssetHelper helper] requestAssetThumbnail:asset completion:^(MNAsset *model) {
        ...
        if (model.type == MNAssetTypePhoto || model.type == MNAssetTypeGif) {
            ...
            self.playView.coverView.image = model.thumbnail;
            self.currentTimeLabel.text = @"00:00";
            self.durationLabel.text = model.durationString ? : @"00:00";
        } else {
            if (@available(iOS 9.1, *)) {
                PHLivePhotoView *livePhotoView = (PHLivePhotoView *)self.livePhotoView;
...
- (void)playerDidEndDecode:(MNPlayer *)player {
    self.durationLabel.text = [NSDate timeStringWithInterval:@(ceil(player.duration))];
}
