...
static NSString * const UIViewPageStatisticalKey = @"com.mn.page.statistical.key";
@interface UIView (MNPageStatistical)
@property (nonatomic) NSInteger mn_pageIndex;
@end

@implementation UIView (MNPageStatistical)

- (void)setMn_pageIndex:(NSInteger)mn_pageIndex {
    objc_setAssociatedObject(self, &UIViewPageStatisticalKey, @(mn_pageIndex), OBJC_ASSOCIATION_RETAIN);
}

- (NSInteger)mn_pageIndex {
    NSNumber *number = objc_getAssociatedObject(self, &UIViewPageStatisticalKey);
...
- (void)layoutSubviews {
    ...
    [subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull view, NSUInteger idx, BOOL * _Nonnull stop) {
        if (view.mn_pageIndex >= numberOfPages) {
    ...
    for (NSUInteger idx = 0; idx < numberOfPages; idx++) {
        ...
        if ([_delegate respondsToSelector:@selector(pageControl:shouldUpdateCell:forPageOfIndex:)] ) {
            [_delegate pageControl:self shouldUpdateCell:page forPageOfIndex:idx];
        } else {
...
- (CGRect)cellRectForPageOfIndex:(NSUInteger)pageIndex {
    NSArray <UIView *>* pages = [self.contentView.subviews filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self.mn_pageIndex == %ld", pageIndex]];
...
- (void)updateCurrentPageIfNeeded {
    ...
    [subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([self.delegate respondsToSelector:@selector(pageControl:shouldUpdateCell:forPageOfIndex:)] ) {
            [self.delegate pageControl:self shouldUpdateCell:obj forPageOfIndex:obj.mn_pageIndex];
        } else {
            obj.backgroundColor = (obj.mn_pageIndex == self.currentPageIndex ? self.currentPageIndicatorTintColor : self.pageIndicatorTintColor);
        }
    }];
}
...
- (void)updateCurrentPageWithTouches:(NSSet<UITouch *> *)touches event:(UIEvent *)event {
    ...
    __block UIView *page;
    [self.contentView.subviews.copy enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (!page) return;
    NSInteger currentPageIndex = page.mn_pageIndex;
    if (currentPageIndex == self.currentPageIndex) return;
