...
/// 判断是否可以追加弹窗的环境变量
static BOOL MNAlertViewAllowsShow = YES;
/// 获取弹窗的Key
...
+ (void)close {
    ...
    BOOL allowsAppend = MNAlertViewAllowsShow;
    MNAlertViewAllowsShow = NO;
    [alertViews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [keyWindow.alertViews_ removeObjectsInArray:alertViews];
    MNAlertViewAllowsShow = allowsAppend;
}
...
- (void)removeFromSuperview {
    ...
    if (!MNAlertViewAllowsShow || superview.alertViews_.count <= 0 || !superview.window) return;
