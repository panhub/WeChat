...
@implementation UIView (MNMenuClose)

- (void)closeMenuView {
    [self closeMenuWithAnimated:YES];
}

- (void)closeMenuWithAnimated:(BOOL)animated {
    MNMenuView *menuView = [self viewWithTag:MNMenuViewTag];
    if (!menuView) return;
    [menuView dismiss:nil animated:animated];
}
