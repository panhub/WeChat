...
#define MNActionSheetButtonHeight         54.f
#define MNActionSheetButtonMargin        (UIScreen.mainScreen.scale < 3.f ? 1.f : .8f)
#define MNActionSheetButtonTitleColor    [UIColor.darkTextColor colorWithAlphaComponent:.85f]
...
+ (instancetype)actionSheetWithTitle:(id)title
    ...
    NSMutableArray *otherButtonTitles = @[].mutableCopy;
...
- (void)createView {
    ...
    contentView.backgroundColor = MN_RGB(247.f);
    [self addSubview:contentView];
    _contentView = contentView;
    
    __block CGFloat margin = 0.f;
    if (self.title) {
        NSMutableAttributedString *title = [self.title isKindOfClass:NSAttributedString.class] ? [self.title mutableCopy] : nil;
        if (!title) {
            title = [[NSMutableAttributedString alloc] initWithString:self.title];
            [title addAttribute:NSFontAttributeName value:[UIFont fontWithName:MNFontNameRegular size:14.f] range:NSMakeRange(0, title.length)];
            [title addAttribute:NSForegroundColorAttributeName value:[UIColor.darkGrayColor colorWithAlphaComponent:.675f] range:NSMakeRange(0, title.length)];
        }
        NSMutableParagraphStyle *style = NSMutableParagraphStyle.new;
        style.alignment = NSTextAlignmentCenter;
        [title addAttribute:NSParagraphStyleAttributeName value:style range:NSMakeRange(0, title.length)];
        UILabel *titleLabel = [UILabel labelWithFrame:CGRectMake(0.f, 0.f, contentView.width_mn, MNActionSheetButtonHeight + floor([title sizeOfLimitWidth:self.contentView.width_mn - 30.f].height)) text:title alignment:NSTextAlignmentCenter textColor:UIColorWithAlpha([UIColor darkGrayColor], .675f) font:UIFontWithNameSize(MNFontNameRegular, 14.f)];
    ...
    if (self.cancelButtonTitle) {
        UIButton *button = [UIButton buttonWithFrame:CGRectMake(0.f, margin + 8.f, contentView.width_mn, MNActionSheetButtonHeight)
                                             image:nil
    ...
    if (MN_TAB_SAFE_HEIGHT > 0.f) {
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0.f, margin, contentView.width_mn, MN_TAB_SAFE_HEIGHT)];
    ...
    [contentView.layer setMaskRadius:15.f byCorners:UIRectCornerTopLeft|UIRectCornerTopRight];
}
