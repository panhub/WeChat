...
- (void)createView {
    ...
    self.containerView.size_mn = CGSizeMake(48.f, 35.f);
    [self.contentView addSubview:self.containerView];
    
    [self.contentView addSubview:self.textLabel];
    
    UIBezierPath *bezierPath = [UIBezierPath bezierPath];
    [bezierPath moveToPoint:CGPointMake(kMNCompletedLineWidth/2.f, self.containerView.height_mn/2.f)];
...
- (void)startAnimation {
    ...
    [self.shapeLayer addAnimation:strokeAnimation forKey:MNLoadDialogAnimationKey];
    [self performSelector:@selector(stopAnimation) withObject:nil afterDelay:1.2f];
}
