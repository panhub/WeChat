...
- (void)showCompletedDialog:(NSString *)message {
    [self showDialog:MNLoadDialogStyleComplete message:message];
}
...
- (void)showDialog:(MNLoadDialogStyle)style message:(NSString *)message {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        if ([dialog showInView:weakself]) {
            [weakself setLoadDialog_:dialog];
        }
    }];
}
...
- (void)closeDialog {
    __weak typeof(self) weakself = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        MNLoadDialog *alert = [weakself loadDialog_];
        if (alert && alert.superview) [alert dismiss];
        [weakself setLoadDialog_:nil];
    }];
}
...
@implementation UIView (MNWechatLoading)

#pragma mark - 微信弹窗
- (void)showWechatDialog {
    [self showDialog:MNLoadDialogStyleWechatLoading message:@"请稍后..."];
}
...
- (void)showWechatDialogNeeds:(BOOL)isNeed delay:(NSTimeInterval)delay eventHandler:(void(^)(void))eventHandler completionHandler:(void(^)(void))completionHandler {
    ...
    if (delay > 0.f) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay/2.f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (eventHandler) eventHandler();
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delay/2.f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [weakself closeDialogWithCompletionHandler:completionHandler];
            });
        });
    } else {
        if (eventHandler) eventHandler();
...
- (void)showWechatError:(NSString *)message {
    [self showDialog:MNLoadDialogStyleWechatError message:message];
}

- (void)showWechatComplete:(NSString *)message {
    [self showDialog:MNLoadDialogStyleWechatComplete message:message];
}
