...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.cornerSize = CGSizeMake(3.f, MNCropTouchEdge);
...
- (void)drawRect:(CGRect)rect {
    ...
    //左上
    CGContextMoveToPoint(context, _cropRect.origin.x + _cornerSize.width/2.f + _cornerSize.height/2.f, _cropRect.origin.y + _cornerSize.width/2.f);
    CGContextAddLineToPoint(context, _cropRect.origin.x + _cornerSize.width/2.f, _cropRect.origin.y + _cornerSize.width/2.f);
    CGContextAddLineToPoint(context, _cropRect.origin.x + _cornerSize.width/2.f, _cropRect.origin.y + _cornerSize.width/2.f + _cornerSize.height/2.f);
    //右上
    CGContextMoveToPoint(context, CGRectGetMaxX(_cropRect) - _cornerSize.width/2.f - _cornerSize.height/2.f, _cropRect.origin.y + _cornerSize.width/2.f);
    CGContextAddLineToPoint(context, CGRectGetMaxX(_cropRect) - _cornerSize.width/2.f, _cropRect.origin.y + _cornerSize.width/2.f);
    CGContextAddLineToPoint(context, CGRectGetMaxX(_cropRect) - _cornerSize.width/2.f, _cropRect.origin.y + _cornerSize.width/2.f + _cornerSize.height/2.f);
    //右下
    CGContextMoveToPoint(context, CGRectGetMaxX(_cropRect) - _cornerSize.width/2.f, CGRectGetMaxY(_cropRect) - _cornerSize.width/2.f - _cornerSize.height/2.f);
    CGContextAddLineToPoint(context, CGRectGetMaxX(_cropRect) - _cornerSize.width/2.f, CGRectGetMaxY(_cropRect) - _cornerSize.width/2.f);
    CGContextAddLineToPoint(context, CGRectGetMaxX(_cropRect) - _cornerSize.width/2.f - _cornerSize.height/2.f, CGRectGetMaxY(_cropRect) - _cornerSize.width/2.f);
    //左下
    CGContextMoveToPoint(context, _cropRect.origin.x + _cornerSize.width/2.f + _cornerSize.height/2.f, CGRectGetMaxY(_cropRect) - _cornerSize.width/2.f);
    CGContextAddLineToPoint(context, _cropRect.origin.x + _cornerSize.width/2.f, CGRectGetMaxY(_cropRect) - _cornerSize.width/2.f);
    CGContextAddLineToPoint(context, _cropRect.origin.x + _cornerSize.width/2.f, CGRectGetMaxY(_cropRect) - _cornerSize.width/2.f - _cornerSize.height/2.f);
    CGContextStrokePath(context);
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectIsEmpty(_cropRect) || CGRectContainsPoint(UIEdgeInsetsInsetRect(_cropRect, UIEdgeInsetWith(-_cornerSize.height/2.f)), location) == NO) {
        self.cropBorder = MNCropBorderNone;
    } /*else if (CGRectContainsPoint(UIEdgeInsetsInsetRect(_cropRect, UIEdgeInsetWith(_cornerSize.height/2.f)), location)) {
       self.cropBorder = MNCropBorderCenter;
       } */else if (CGRectContainsPoint(CGRectCenterSide(_cropRect.origin, _cornerSize.height), location)) {
           self.cropBorder = MNCropBorderTop|MNCropBorderLeft;
       } else if (CGRectContainsPoint(CGRectCenterSide(CGPointMake(CGRectGetMaxX(_cropRect), CGRectGetMinY(_cropRect)), _cornerSize.height), location)) {
           self.cropBorder = MNCropBorderTop|MNCropBorderRight;
       } else if (CGRectContainsPoint(CGRectCenterSide(CGPointMake(CGRectGetMaxX(_cropRect), CGRectGetMaxY(_cropRect)), _cornerSize.height), location)) {
           self.cropBorder = MNCropBorderBottom|MNCropBorderRight;
       } else if (CGRectContainsPoint(CGRectCenterSide(CGPointMake(CGRectGetMinX(_cropRect), CGRectGetMaxY(_cropRect)), _cornerSize.height), location)) {
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    CGFloat minInterval = _cornerSize.height + _cornerSize.width;
    if (self.scale > 0.f) {
        // 按照比例裁剪
...
- (void)setCornerSize:(CGSize)cornerSize {
    _cornerSize.width = MAX(cornerSize.width, _borderWidth*2.f);
    _cornerSize.height = MAX(cornerSize.height, 30.f);
    [self setNeedsDisplay];
}
