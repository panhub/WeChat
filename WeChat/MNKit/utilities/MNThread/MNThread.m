...
static NSString *MNThreadNameGet (void) {
...
@property (nonatomic, copy) MNThreadTaskCallback taskCallback;
@end
...
/**是否保持RunLoop运行*/
@property (nonatomic, assign) BOOL isKeepRun;
/**RunLoop运行哪个模型*/
@property (nonatomic, copy, readwrite) NSRunLoopMode runLoopMode;
@end
...
- (instancetype)init {
    ...
    if (!self) return nil;
    self.isKeepRun = YES;
    _mode = MNDefaultRunLoopMode;
    self.runLoopMode = NSDefaultRunLoopMode;
    NSThread *thread = [[NSThread alloc] initWithTarget:self selector:@selector(run) object:nil];
    thread.name = MNThreadNameGet();
    [thread start];
    self.thread = thread;
    return self;
}
...
- (void)setMode:(MNRunLoopMode)mode {
    if (mode == _mode) return;
    if (!_thread) return;
    _mode = mode;
    NSRunLoopMode runLoopMode = mode == MNDefaultRunLoopMode ? NSDefaultRunLoopMode : UITrackingRunLoopMode;
