...
NSString * const MNKeyChainErrorDomain = @"com.mn.kishikawakatsumi.keychain";
static NSString *_defaultService;

@interface MNKeyChain ()
...
+ (MNKeyChain *)keyChainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (MNKeyChain *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:MNKeyChainAuthenticationTypeDefault];
}
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        _itemClass = MNKeyChainItemClassGenericPassword;
...
{
    return [self initWithServer:server protocolType:protocolType authenticationType:MNKeyChainAuthenticationTypeDefault];
}
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(MNKeyChainProtocolType)protocolType authenticationType:(MNKeyChainAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = MNKeyChainItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = MNKeyChainAccessibilityAfterFirstUnlock;
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    MNKeyChain *keychain = [MNKeyChain keyChainStoreWithService:service accessGroup:accessGroup];
...
+ (NSArray UIC_KEY_TYPE *)allKeysWithItemClass:(MNKeyChainItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == MNKeyChainItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == MNKeyChainItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(MNKeyChainItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == MNKeyChainItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key value:(NSData *)value error:(NSError *__autoreleasing *)error
{
    ...
    if (_authenticationPolicy && accessibilityObject) {
        ...
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == MNKeyChainAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            NSLog(@"%@", @"Unavailable 'MNKeyChainAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@", @"Unavailable 'MNKeyChainAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case MNKeyChainItemClassGenericPassword:
            return kSecClassGenericPassword;
        case MNKeyChainItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case MNKeyChainProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case MNKeyChainProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case MNKeyChainProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case MNKeyChainProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case MNKeyChainProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case MNKeyChainProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case MNKeyChainProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case MNKeyChainProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case MNKeyChainProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case MNKeyChainProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case MNKeyChainProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case MNKeyChainProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case MNKeyChainProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case MNKeyChainProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case MNKeyChainProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case MNKeyChainProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case MNKeyChainProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case MNKeyChainProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case MNKeyChainProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case MNKeyChainProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case MNKeyChainProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case MNKeyChainProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case MNKeyChainProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case MNKeyChainProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case MNKeyChainProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case MNKeyChainProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case MNKeyChainProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case MNKeyChainProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case MNKeyChainProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case MNKeyChainAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case MNKeyChainAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case MNKeyChainAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case MNKeyChainAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case MNKeyChainAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case MNKeyChainAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case MNKeyChainAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case MNKeyChainAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case MNKeyChainAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case MNKeyChainAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case MNKeyChainAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case MNKeyChainAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case MNKeyChainAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case MNKeyChainAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case MNKeyChainAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:MNKeyChainErrorDomain code:MNKeyChainErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:MNKeyChainErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:MNKeyChainErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:MNKeyChainErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
@implementation MNKeyChain (Deprecation)
