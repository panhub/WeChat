...
void mn_url_session_create_task_safely (dispatch_block_t block) {
...
@property (nonatomic, assign) MNURLResponseSerializationType serializationType;
...
#pragma clang diagnostic ignored "-Wgnu"
    ...
    if (error) {
        ...
        dispatch_async(mn_url_session_create_serialization_queue(), ^{
            ...
            responseObject = [session.responseSerializer responseObjectForResponse:task.response type:self.serializationType data:data error:&serializationError];
            //如果是下载文件，那么responseObject为下载的路径
            if (self.downloadURL) {
                responseObject = self.downloadURL;
            }
            //回调结果
            dispatch_group_async(session.completionGroup ?: mn_url_session_create_completion_group(), session.completionQueue ?: dispatch_get_main_queue(), ^{
                if (self.completionCallback) {
    ...
    didFinishDownloadingToURL:(NSURL *)location
{
    ...
    if (self.downloadPathCallback) {
        NSURL *downloadURL = self.downloadPathCallback(downloadTask.response, location);
        if (downloadURL) {
            if ([[NSFileManager defaultManager] moveItemAtURL:location toURL:downloadURL error:nil]) {
...
@property (readwrite, nonatomic, strong) MNURLRequestSerializer *requestSerializer;
@property (readwrite, nonatomic, strong) MNURLResponseSerializer *responseSerializer;
...
{
    dispatch_semaphore_t _lock; //信号量锁
    CFMutableDictionaryRef _delegateContainerRef; //代理容器
}
...
- (instancetype)initWithSessionConfiguration:(nullable NSURLSessionConfiguration *)configuration {
    ...
    self.requestSerializer = [MNURLRequestSerializer serializer];
    self.responseSerializer = [MNURLResponseSerializer serializer];
    ...
    [self.session getTasksWithCompletionHandler:^(NSArray *dataTasks, NSArray *uploadTasks, NSArray *downloadTasks) {
        for (NSURLSessionDataTask *dataTask in dataTasks) {
            [self setDelegateForDataTask:dataTask uploadProgress:nil downloadProgress:nil completion:nil];
        }
        for (NSURLSessionUploadTask *uploadTask in uploadTasks) {
            [self setDelegateForDataTask:uploadTask uploadProgress:nil downloadProgress:nil completion:nil];
        }
        for (NSURLSessionDownloadTask *downloadTask in downloadTasks) {
            [self setDelegateForDownloadTask:downloadTask downloadProgress:nil downloadPath:nil completion:nil];
        }
    }];
...
{
    return [self dataTaskWithUrl:url
                          method:@"GET"
                       parameter:parameter
                  uploadProgress:nil
...
- (NSURLSessionDataTask *)POST:(NSString *)url
                ...
                uploadProgress:(MNURLSessionTaskProgressCallback)uploadProgress
                       completion:(MNURLSessionTaskCompleteCallback)completion
{
    NSURLSessionDataTask *dataTask = [self dataTaskWithUrl:url
                                                     method:@"POST"
                                                  parameter:parameter
                              ...
                              completion:(MNURLSessionTaskCompleteCallback)completion
{
    ...
    /**保存代理*/
    [self setDelegateForDataTask:dataTask uploadProgress:uploadProgress downloadProgress:downloadProgress completion:completion];
    [dataTask resume];
    return dataTask;
}
                                   ...
                                   completion:(MNURLSessionTaskCompleteCallback)completion
{
    ...
    if (uploadPath) {
        uploadPath(&fileURL, &fileData);
    }
    if (fileData) {
        mn_url_session_create_task_safely(^{
            uploadTask = [self.session uploadTaskWithRequest:request fromData:fileData];
        });
    } else if (fileURL) {
    ...
    if (!uploadTask) return nil;
    /**保存代理*/
    [self setDelegateForDataTask:uploadTask uploadProgress:uploadProgress downloadProgress:nil completion:completion];
    [uploadTask resume];
    return uploadTask;
}
                                       ...
                                       completion:(MNURLSessionTaskCompleteCallback)completion
{
    ...
    /**下载回调*/
    [self setDelegateForDownloadTask:downloadTask downloadProgress:downloadProgress downloadPath:downloadPath completion:completion];
    [downloadTask resume];
    return downloadTask;
}
                    ...
                    completion:(MNURLSessionTaskCompleteCallback)completion
{
    ...
    delegate.serializationType = self.responseSerializer.type;
        ...
        didFinishDownloadingToURL:(NSURL *)location
{
    ...
    if (self.downloadTaskDidFinishDownloadingCallback) {
        ...
        NSURL *fileURL = self.downloadTaskDidFinishDownloadingCallback(session, downloadTask, location);
        if (fileURL) {
            NSError *fileManagerError;
            if ([[NSFileManager defaultManager] moveItemAtURL:location toURL:fileURL error:&fileManagerError]) {
                delegate.downloadURL = fileURL;
            } else {
                if (self.downloadTaskMoveFileFailedCallback) {
