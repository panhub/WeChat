...
@implementation MNTableField
@end

@interface MNDatabase ()
@end

MNTableFieldType const MNTableFieldText = @"TEXT";
MNTableFieldType const MNTableFieldBlob = @"BLOB";
MNTableFieldType const MNTableFieldReal = @"REAL";
MNTableFieldType const MNTableFieldInteger = @"INTEGER";

static MNDatabase *_manager;
...
@implementation MNDatabase
{
    ...
    NSMutableDictionary <NSString *, NSArray <MNTableField *>*>*_tableInfoCache;
...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        if (!_manager) {
            _manager = [[MNDatabase alloc] init];
        }
    });
    return _manager;
}
...
- (instancetype)init {
    ...
    dispatch_once(&onceToken, ^{
        _manager = [super init];
        if (_manager) {
            _dbPath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"mn-database.sqlite"];
            ...
            NSArray <NSString *>*tables = [_manager selectTables];
            if (tables.count) [_tableNameCache addObjectsFromArray:tables];
            NSLog(@"default.db.path===%@", _dbPath);
        }
    });
    return _manager;
}
...
+ (void)createTable:(NSString *)name fields:(NSDictionary <NSString *, NSString *>*)fields completion:(void(^)(BOOL))completion {
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] createTable:name fields:fields];
        if (completion) {
            completion(succeed);
        }
    });
...
+ (void)createTable:(NSString *)name class:(Class)cls completion:(void(^)(BOOL))completion {
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] createTable:name class:cls];
        if (completion) {
            completion(succeed);
        }
    });
...
+ (void)createTable:(NSString *)name completion:(void(^)(BOOL))completion fieldsAndTypes:(NSString *)field,...NS_REQUIRES_NIL_TERMINATION {
    ...
    va_end(args);
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] createTable:name fields:dic.copy];
        if (completion) {
            completion(succeed);
        }
    });
...
- (BOOL)deleteTable:(NSString *)name {
    ...
    if (succeed) {
        ...
        [_tableNameCache removeObjectsInArray:tableNames];
    }
    Unlock();
    return succeed;
}
...
+ (void)deleteTable:(NSString *)name completion:(void(^)(BOOL))completion {
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] deleteTable:name];
        if (completion) {
            completion(succeed);
        }
    });
...
+ (void)insertIntoTable:(NSString *)name fields:(NSDictionary <NSString *, id>*)fields completion:(void(^)(BOOL))completion {
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] insertIntoTable:name fields:fields];
        if (completion) {
            completion(succeed);
        }
    });
...
- (BOOL)insertIntoTable:(NSString *)name model:(id)model {
    return [self insertIntoTable:name fields:[self fieldsAndValuesOfModel:model]];
}

+ (void)insertIntoTable:(NSString *)name model:(id)model completion:(void(^)(BOOL))completion
{
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] insertIntoTable:name model:model];
        if (completion) {
            completion(succeed);
        }
    });
...
- (BOOL)insertIntoTable:(NSString *)name fieldsAndValues:(id)field,...NS_REQUIRES_NIL_TERMINATION {
    ...
    va_end(args);
    return [self insertIntoTable:name fields:dic.copy];
}

+ (void)insertIntoTable:(NSString *)name completion:(void(^)(BOOL))completion fieldsAndValues:(id)field,...NS_REQUIRES_NIL_TERMINATION {
    ...
    va_end(args);
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] insertIntoTable:name fields:dic.copy];
        if (completion) {
            completion(succeed);
        }
    });
...
+ (void)updateTable:(NSString *)name where:(NSString *)where fields:(NSDictionary <NSString *, id>*)fields completion:(void(^)(BOOL))completion {
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] updateTable:name where:where fields:fields];
        if (completion) {
            completion(succeed);
        }
    });
...
+ (void)updateTable:(NSString *)name where:(NSString *)where model:(id)model completion:(void(^)(BOOL))completion {
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] updateTable:name where:where model:model];
        if (completion) {
            completion(succeed);
        }
    });
...
+ (void)updateTable:(NSString *)name where:(NSString *)where completion:(void(^)(BOOL))completion fieldsAndValues:(id)field,...NS_REQUIRES_NIL_TERMINATION {
    ...
    va_end(args);
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] updateTable:name where:where fields:dic.copy];
        if (completion) {
            completion(succeed);
        }
    });
...
+ (void)deleteRowFromTable:(NSString *)name where:(NSString *)where completion:(void(^)(BOOL))completion {
    dispatch_async(dispatch_database_queue, ^{
        BOOL succeed = [[MNDatabase sharedInstance] deleteRowFromTable:name where:where];
        if (completion) {
            completion(succeed);
        }
    });
...
+ (void)selectTablesUsingBlock:(void(^)(NSArray <NSString *>*))block {
    if (!block) return;
    dispatch_async(dispatch_database_queue, ^{
        NSArray <NSString *>*tables = [[MNDatabase sharedInstance] selectTables];
        if (block) {
            block(tables);
        }
    });
...
+ (void)selectCountFromTable:(NSString *)name where:(NSString *)where completion:(void(^)(NSUInteger))completion {
    ...
    dispatch_async(dispatch_database_queue, ^{
        NSUInteger count = [[MNDatabase sharedInstance] selectCountFromTable:name where:where];
        if (completion) {
            completion(count);
        }
    });
...
- (CGFloat)selectColumnSumFromTable:(NSString *)name field:(NSString *)field {
    if (name.length <= 0 || field.length <= 0 || ![self existsTable:name] || ![self dbOpen]) return 0.f;
    NSString *sql = [NSString stringWithFormat:@"SELECT SUM(%@) FROM %@;", field, name];
    Lock();
    CGFloat sum = 0.f;
    sqlite3_stmt *stmt = [self sqlStmt:sql];
    if (stmt && sqlite3_step(stmt) == SQLITE_ROW) {
...
+ (void)selectColumnSumFromTable:(NSString *)name field:(NSString *)field completion:(void(^)(CGFloat))completion {
    if (!completion) return;
    dispatch_async(dispatch_database_queue, ^{
        CGFloat sum = [[MNDatabase sharedInstance] selectColumnSumFromTable:name field:field];
        if (completion) {
            completion(sum);
        }
    });
...
- (nullable NSArray <NSDictionary <NSString *, id>*>*)selectRowsFromTable:(NSString *)name sql:(NSString *)sql {
    ...
    NSArray <MNTableField *>*infos = [self selectTableInfo:name];
        ...
        do {
            ...
            if (result == SQLITE_ROW) {
                ...
                for (int i = 0; i < count; i++) {
                    MNTableField *info = [self tableInfoInFields:nil infos:infos idx:i];
                    if (!info) continue;
                    NSString *type = info.type;
                    if ([type isEqualToString:MNTableFieldText]) {
                        const char *value = (const char *)sqlite3_column_text(stmt, i);
                        NSString *string = [NSString stringWithUTF8String:value];
                        if (!string) string = @"";
                        [dic setObject:string forKey:info.field];
                    } else if ([type isEqualToString:MNTableFieldBlob]) {
                    ...
                    } else if ([type isEqualToString:MNTableFieldReal]) {
                        double num = sqlite3_column_double(stmt, i);
                        [dic setObject:@(num) forKey:info.field];
                    } else if ([type isEqualToString:MNTableFieldInteger]) {
...
+ (void)selectRowsFromTable:(NSString *)name sql:(NSString *)sql completion:(void(^)(NSArray <NSDictionary <NSString *, id>*>* rows))completion {
    if (!completion) return;
    dispatch_async(dispatch_database_queue, ^{
        NSArray <NSDictionary <NSString *, id>*>*rows = [[MNDatabase sharedInstance] selectRowsFromTable:name sql:sql];
        if (completion) {
            completion(rows);
        }
    });
...
+ (void)selectRowsModelFromTable:(NSString *)name sql:(NSString *)sql class:(Class)cls completion:(void(^)(NSArray <id>* rows))completion {
    if (!completion) return;
    dispatch_async(dispatch_database_queue, ^{
        NSArray <id>* rows = [[MNDatabase sharedInstance] selectRowsModelFromTable:name sql:sql class:cls];
        if (completion) {
            completion(rows);
        }
    });
    ...
    if (stmt) {
        ...
        do {
            ...
            if (result == SQLITE_ROW) {
                ...
                NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithCapacity:0];
                for (int i = 0; i < count; i++) {
                    MNTableField *info = [self tableInfoInFields:fields infos:infos idx:i];
                    if (!info) continue;
                    NSString *type = info.type;
                    if ([type isEqualToString:MNTableFieldText]) {
                        const char *value = (const char *)sqlite3_column_text(stmt, i);
...
- (MNTableField *)tableInfoInFields:(NSArray <NSString *>*)fields infos:(NSArray <MNTableField *>*)infos idx:(NSInteger)idx {
    if (fields.count <= 0) return infos[idx];
    NSString *field = fields[idx];
    __block MNTableField *info;
    [infos enumerateObjectsUsingBlock:^(MNTableField * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (void)selectRowsFromTable:(NSString *)name fields:(NSArray <NSString *>*)fields where:(NSString *)where limit:(NSRange)limit completion:(void(^)(NSArray <NSDictionary <NSString *, id>*>*))completion {
    if (!completion) return;
    dispatch_async(dispatch_database_queue, ^{
        NSArray <NSDictionary <NSString *, id>*>*rows = [[MNDatabase sharedInstance] selectRowsFromTable:name fields:fields where:where limit:limit];
        if (completion) {
            completion(rows);
        }
    });
...
+ (void)selectRowsModelFromTable:(NSString *)name where:(NSString *)where limit:(NSRange)limit class:(Class)cls completion:(void(^)(NSArray <id>*))completion {
    if (!completion) return;
    dispatch_async(dispatch_database_queue, ^{
        NSArray <id>* rows = [[MNDatabase sharedInstance] selectRowsModelFromTable:name where:where limit:limit class:cls];
        if (completion) {
            completion(rows);
        }
    });
...
/// 获取表中所有字段名类型
- (NSArray <MNTableField *>*)selectTableInfo:(NSString *)name {
    if (name.length <= 0) return nil;
    Lock();
    NSArray <MNTableField *>*infos = _tableInfoCache[name];
    if (!infos) {
        sqlite3_stmt *stmt;
        NSMutableArray <MNTableField *>*array = [NSMutableArray arrayWithCapacity:0];
        ...
        if (sqlite3_prepare_v2(_db, sql.UTF8String, -1, &stmt, NULL) == SQLITE_OK) {
            ...
            do {
                ...
                if (result == SQLITE_ROW) {
                    ...
                    MNTableField *info = [MNTableField new];
                    info.idx = idx;
...
- (NSDictionary <NSString *, NSString *>*)fieldsOfClass:(Class)cls {
    NSDictionary <NSString *, NSString *>*fields = [self fieldsOfClassFromCache:cls];
    ...
    fields = dic.copy;
    if (fields.count > 0) {
        [self setFieldsToCache:fields ofClass:cls];
    }
    return fields;
}

- (NSDictionary <NSString *, NSString *>*)fieldsOfClassFromCache:(Class)cls {
    Lock();
...
- (void)setFieldsToCache:(NSDictionary <NSString *, NSString *>*)fields ofClass:(Class)cls {
    Lock();
...
- (void)bindSqlStmt:(sqlite3_stmt *)stmt values:(NSArray *)values {
    for (int idx = 1; idx <= values.count; idx++) {
        ...
        if ([obj isKindOfClass:NSString.class]) {
            ...
            int intv = ((NSNumber *)obj).intValue;
            double dou = ((NSNumber *)obj).doubleValue;
            if (fabs(dou - intv) == 0.f) {
                sqlite3_bind_int(stmt, idx, intv);
            } else {
