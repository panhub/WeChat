...
dispatch_queue_t dispatch_create_queue (NSString *name, dispatch_queue_attr_t attr) {
    return dispatch_queue_create(MNQueueNameCheck(name).UTF8String, attr);
}

#pragma mark - 创建串行队列
dispatch_queue_t dispatch_create_serial_queue (NSString *name) {
    return dispatch_queue_create(MNQueueNameCheck(name).UTF8String, DISPATCH_QUEUE_SERIAL);
}

#pragma mark - 创建并行队列
dispatch_queue_t dispatch_create_concurrent_queue (NSString *name) {
    return dispatch_queue_create(MNQueueNameCheck(name).UTF8String, DISPATCH_QUEUE_CONCURRENT);
}
...
#pragma mark - 存放定时器
void dispatch_timer_save (_Nullable dispatch_source_t timer, NSString *name) {
    if (name.length <= 0) return;
