...
const MNFontWeight MNFontWeightThin = -0.6f;
const MNFontWeight MNFontWeightLight = -0.4f;
const MNFontWeight MNFontWeightRegular = 0.f;
const MNFontWeight MNFontWeightMedium = 0.23f;
const MNFontWeight MNFontWeightBold = 0.4f;
...
+ (NSString *)registFontWithPath:(NSString *)path {
    ...
    if (CTFontManagerRegisterGraphicsFont(fontRef, &errorRef)) {
        ...
        NSString *name = (__bridge_transfer NSString *)CGFontCopyFullName(fontRef);
        if (!name) name = @"";
        NSError *error = (__bridge_transfer NSError *)errorRef;
        NSLog(@"regist font failed!\n---name: %@\n---path: %@\n---error: %@", name, path, error.userInfo);
    }
