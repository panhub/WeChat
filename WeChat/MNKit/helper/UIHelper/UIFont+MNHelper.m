...
+ (UIFont *)mn_fontWithName:(NSString *)fontName size:(CGFloat)fontSize {
    ...
    if ([self registFontWithPath:path]) {
        font = [UIFont mn_fontWithName:fontName size:fontSize];
    } else {
        NSLog(@"regist font error - fontName:%@, fontSize:%.2f",fontName, fontSize);
...
+ (BOOL)registFontWithPath:(NSString *)path {
    ...
    CFErrorRef error;
    BOOL succeed = CTFontManagerRegisterGraphicsFont(fontRef, &error);
    if (succeed) {
...
+ (UIFont *)systemFontOfSizes:(CGFloat)fontSize weights:(CGFloat)weight {
