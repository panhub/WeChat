...
#pragma mark - 判断当前系统版本是否高于某个版本
inline BOOL IOS_MAX (CGFloat version) {
    return IOS_VERSION_NUMBER() > version;
}

#pragma mark - 判断当前系统版本是否低于某个版本
inline BOOL IOS_MIN (CGFloat version) {
    return IOS_VERSION_NUMBER() < version;
}
...
#pragma mark - 屏幕界限
inline CGRect UIScreenBounds (void) {
    return [[UIScreen mainScreen] bounds];
}

#pragma mark - 屏幕尺寸
inline CGSize UIScreenSize (void) {
...
CGFloat UIScreenMax (void) {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        max = MAX(UIScreenWidth(), UIScreenHeight());
    });
    return max;
}
...
CGFloat UIScreenMin (void) {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        min = MIN(UIScreenWidth(), UIScreenHeight());
    });
    return min;
}

#pragma mark - 底部安全区域高度
CGFloat UITabSafeHeight (void) {
    static CGFloat safeareaheight = 0.f;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (UIInterfacePhoneModel() && UIStatusBarHeight() > 20.f) {
            safeareaheight = 34.f;
        }
    });
...
#pragma mark - 底部标签控制栏高度
CGFloat UITabBarHeight (void) {
    static CGFloat tabbarheight = 0.f;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // 为微信适配
        tabbarheight = 56.f + UITabSafeHeight();
        //UITabBarController *tabBarController = [UITabBarController new];
        //tabbarheight = CGRectGetHeight(tabBarController.tabBar.frame) + UITabSafeHeight();
    });
    return tabbarheight;
}
...
#pragma mark - NavigationBar高度
CGFloat UINavBarHeight (void) {
    static CGFloat height = 0.f;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        UINavigationController *nav = [UINavigationController new];
        height = CGRectGetHeight(nav.navigationBar.frame);
    });
    return height;
}

#pragma mark - 状态栏与导航栏高度
CGFloat UITopBarHeight (void) {
    static CGFloat topbarheight;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        topbarheight = UIStatusBarHeight() + UINavBarHeight();
    });
    return topbarheight;
}
