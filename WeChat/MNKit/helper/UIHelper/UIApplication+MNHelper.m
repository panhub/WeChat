...
+ (void)handOpenUrl:(id)url completion:(UIApplicationOpenHandler)handler {
    ...
    if (!URL) UIApplicationOpenCallback(NO);
    if (__IPHONE_OS_VERSION_MAX_ALLOWED >= 100000) {
        ...
        BOOL handOpen = [[UIApplication sharedApplication] openURL:URL];
        UIApplicationOpenCallback(handOpen);
    } else {
        UIApplicationOpenCallback(NO);
    }
}
