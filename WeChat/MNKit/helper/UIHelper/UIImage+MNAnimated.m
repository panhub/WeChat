...
+ (UIImage *)animatedImageWithData:(NSData *)data {
    ...
    if (count <= 1) return [UIImage imageWithData:data];
...
+ (NSData *)dataWithAnimatedImage:(UIImage *)image {
    return [NSData dataWithAnimatedImage:image duration:0.f loopCount:image.loopCount];
}
...
+ (NSData *)dataWithImage:(UIImage *)image {
    if (!image) return nil;
    if (image.isAnimatedImage) return [NSData dataWithAnimatedImage:image];
...
NSData *UIImageGIFRepresentation(UIImage *image) {
    return [NSData dataWithAnimatedImage:image];
}
