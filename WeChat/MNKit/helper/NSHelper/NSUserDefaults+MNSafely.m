...
#pragma mark -
- (BOOL)setImage:(UIImage *)image forKey:(NSString *)defaultName {
    if (!defaultName) return NO;
    NSData *imageData = nil;
    if (image) imageData = UIImageJPEGRepresentation(image, 1.f) ? : UIImagePNGRepresentation(image);
    if (image && !imageData) return NO;
    [self setObject:imageData forKey:defaultName];
