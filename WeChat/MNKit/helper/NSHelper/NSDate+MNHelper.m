...
static NSDateFormatter * NSDateFormatterNormal (void) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [date_formatter_normal setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    });
    return date_formatter_normal;
}

static NSDateFormatter * NSDateFormatterTime (void) {
...
+ (NSString *)timestamps {
    return [[NSNumber numberWithInteger:[self timestamp]] stringValue];
}
...
+ (NSString *)shortTimestamps {
    return [[NSNumber numberWithLongLong:[self shortTimestamp]] stringValue];
}
...
- (NSString *)stringValueWithFormat:(id)obj {
...
+ (NSString *)timeStringWithInterval:(id)obj {
    NSInteger timestamp = NSDateTimestampFrom(obj);
    NSDateFormatter *dateFormatter = NSDateFormatterTime();
    if (timestamp >= 3600) {
...
- (NSDateComponents *)dateComponentSince:(id)obj {
    ...
    NSDateFormatter *dateFormatter = NSDateFormatterNormal();
