...
- (void)didFinishWithResponseObject:(id)responseObject error:(NSError *)error {
    /**更新数据*/
    if (!responseObject && self.method == MNURLHTTPMethodGet && self.dataSource == MNURLDataSourceNetwork && self.cachePolicy == MNURLCachePolicyElseLoad) {
    ...
    if (response.code == MNURLResponseCodeSucceed) {
        if (self.cachePolicy != MNURLCachePolicyNever && self.method == MNURLHTTPMethodGet && self.dataSource == MNURLDataSourceNetwork) {
