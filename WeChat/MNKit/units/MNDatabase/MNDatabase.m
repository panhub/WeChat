...
+ (void)execute:(NSString *)sql completion:(void(^)(BOOL))completion {
    dispatch_async(dispatch_database_queue, ^{
        BOOL result = [MNDatabase.database execute:sql];
...
- (void)bindSqlStmt:(sqlite3_stmt *)stmt values:(NSArray *)values {
    for (int idx = 1; idx <= values.count; idx++) {
        ...
        if ([obj isKindOfClass:NSString.class]) {
            ...
            if (ceil(((NSNumber *)obj).doubleValue) == (double)((NSNumber *)obj).longLongValue) {
