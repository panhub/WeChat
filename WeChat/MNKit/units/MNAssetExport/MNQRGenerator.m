...
#import "MNQRGenerator.h"
#import <CoreImage/CoreImage.h>

@implementation MNQRGenerator
...
+ (void)generateCodeWithMetadata:(NSData *)metadata
                                   pixel:(CGFloat)pixel
                              completion:(void(^)(UIImage *image))completion
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImage *image = [self generateCodeWithMetadata:metadata pixel:pixel];
...
+ (UIImage *)generateCodeWithMetadata:(NSData *)metadata color:(UIColor *)color pixel:(CGFloat)pixel {
    UIImage *image = [self generateCodeWithMetadata:metadata pixel:pixel];
    if (color) image = [self resizingCode:image withColor:color];
    return image;
}
...
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImage *image = [self generateCodeWithMetadata:metadata color:color pixel:pixel];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (completion) {
                completion(image);
            }
...
+ (void)resizingCode:(UIImage *)img withColor:(UIColor *)color completion:(void(^)(UIImage *, UIColor *))completion {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImage *image = [self resizingCode:img withColor:color];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (completion) {
...
+ (UIImage *)insertImageToFrontAtCode:(UIImage *)QRCodeImage image:(UIImage *)image size:(CGSize)size {
...
+ (void)insertImageToFrontAtCode:(UIImage *)QRCodeImage image:(UIImage *)image size:(CGSize)size completion:(void(^)(UIImage *))completion {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImage *qrImage = [MNQRGenerator insertImageToFrontAtCode:QRCodeImage image:image size:size];
        dispatch_async(dispatch_get_main_queue(), ^{
            if (completion) {
                completion(qrImage);
            }
