...
/**
 获取CollectionView布局对象<不提倡使用, 最好直接获取collectionView.collectionViewLayout>
 先判断CollectionView是否存在约束对象, 存在则返回, 不存在则触发创建返回
 不应该重写, 请重写-collectionViewLayout创建具体的约束对象
 */
@property (nonatomic, readonly) __kindof UICollectionViewLayout *collectionLayout;
/**
