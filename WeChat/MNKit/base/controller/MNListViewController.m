...
- (void)displayLoadFooterIfNeeded {
    ...
    MJRefreshFooter *footer = [[NSClassFromString(self.refreshFooterClass) alloc] init];
...
- (BOOL)scrollToBottomWithAnimated:(BOOL)animated {
    if (!_listView.isScrollEnabled || _listView.contentSize.height <= _listView.height_mn) return NO;
