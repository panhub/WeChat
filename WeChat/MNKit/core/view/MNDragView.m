...
- (void)handPan:(UIPanGestureRecognizer *)pan {
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            CGRect frame = rect;
            if (self.centerX_mn >= CGRectGetMinX(_bounding) + CGRectGetWidth(_bounding)/2.f) {
...
- (void)setTimeoutInterval:(NSTimeInterval)timeoutInterval {
    ...
    if (timeoutInterval <= 0.f) {
        [self cancelSleepRequests];
        if (self.alpha != 1.f) self.alpha = 1.f;
    }
}
...
- (void)didMoveToSuperview {
    ...
    if (self.superview && CGRectIsEmpty(self.bounding)) self.bounding = self.superview.bounds;
}
