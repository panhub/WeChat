...
- (void)setControllers:(NSArray<NSString *> *)controllers {
    ...
    if (self.childViewControllers.count) [self.childViewControllers makeObjectsPerformSelector:@selector(removeFromParentViewController)];
...
- (void)addChildViewControllers:(NSArray<NSString *> *)childControllers {
    ...
    [childControllers enumerateObjectsUsingBlock:^(NSString * _Nonnull controller, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        Class cls = [self navigationClassAtIndex:idx];
        
        if (cls != NULL) {
            UINavigationController *navigationController = ((UINavigationController *(*)(id, SEL))objc_msgSend)(cls, sel_registerName("alloc"));
            viewController = ((UINavigationController *(*)(UINavigationController *, SEL, UIViewController *))objc_msgSend)(navigationController, @selector(initWithRootViewController:), viewController);
        }
...
- (nullable id <UIViewControllerAnimatedTransitioning>)tabBarController:(UITabBarController *)tabBarController
    ...
    return [self tabBarControllerTransitionForOperation:operation fromViewController:fromVC toViewController:toVC];
};

#pragma mark - 控制屏幕旋转方法
- (BOOL)shouldAutorotate {
    if (self.selectedViewController) return self.selectedViewController.shouldAutorotate;
    return NO;
}
- (UIInterfaceOrientationMask)tabBarControllerSupportedInterfaceOrientations:(UITabBarController *)tabBarController {
    if (self.selectedViewController) return self.selectedViewController.supportedInterfaceOrientations;
