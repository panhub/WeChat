...
- (void)loadData {
    dispatch_async_default(^{
        NSInteger count = [MNDatabase.sharedInstance selectCountFromTable:self.session.list where:nil];
        ...
        NSArray <WXMessage *>*rows = [MNDatabase.sharedInstance selectRowsModelFromTable:self.session.list sql:sql class:WXMessage.class];
        dispatch_async_main(^{
            if (rows.count > 0) {
                NSMutableArray <WXMessageViewModel *>*listArray = [NSMutableArray arrayWithCapacity:rows.count];
                [rows.reverseObjects enumerateObjectsUsingBlock:^(WXMessage * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
