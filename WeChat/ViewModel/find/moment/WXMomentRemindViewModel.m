...
- (instancetype)initWithModel:(WXMomentRemind *)model {
    if (self = [super init]) {
        self.model = model;
        NSArray <WXMoment *>*rows = [[MNDatabase sharedInstance] selectRowsModelFromTable:WXMomentTableName where:[@{sql_field(model.identifier):sql_pair(model.moment)} componentString] limit:NSRangeZero class:WXMoment.class];
        ...
        WXUser *from_user = [[MNChatHelper helper] userForUid:model.from_uid];
        
        self.headViewModel.frame = CGRectMake(margin, margin, 45.f, 45.f);
        self.headViewModel.content = from_user.avatar;
        
        NSMutableAttributedString *attributedString;
        self.briefViewModel.frame = CGRectMake(SCREEN_WIDTH - (WXMomentRemindCellHeight - margin*2.f) - margin, margin, WXMomentRemindCellHeight - margin*2.f, WXMomentRemindCellHeight - margin*2.f);
        ...
        if (model.content.length) {
            ...
            if (model.to_uid.length && ![model.to_uid isEqualToString:[[WXUser shareInfo] uid]]) {
                ...
                WXUser *to_user = [[MNChatHelper helper] userForUid:model.to_uid];
        ...
        NSString *timeString =  [MNChatHelper momentCreatedTimeWithTimestamp:model.date];
