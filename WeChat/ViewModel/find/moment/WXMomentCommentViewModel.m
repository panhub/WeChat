...
- (instancetype)initWithComment:(WXMomentComment *)comment {
    if (self = [super init]) {
        ...
        NSString *from = [[[MNChatHelper helper] userForUid:comment.from_uid] name];
        NSStringReplaceBlankString(&from);
        ...
        NSString *to = [[[MNChatHelper helper] userForUid:comment.to_uid] name];
        if (to.length > 0) {
