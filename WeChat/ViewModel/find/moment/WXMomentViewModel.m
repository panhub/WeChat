...
- (instancetype)initWithMoment:(WXMoment *)moment {
    if (self = [super init]) {
        ...
        WXUser *user = [[MNChatHelper helper] userForUid:moment.uid];
...
- (void)replyMomentWithModel:(WXMomentReplyViewModel *)replyModel {
    ...
    if (row == NSIntegerMin) {
        ...
        [MNDatabase insertIntoTable:WXMomentCommentTableName model:comment completion:^(BOOL succeed) {
            if (!succeed) return;
...
- (void)insertLikesWithCount:(NSUInteger)count {
    ...
    [[[MNChatHelper helper] contacts] enumerateObjectsUsingBlock:^(WXUser * _Nonnull user, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSAttributedString *)createdTimeString {
    NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:[MNChatHelper momentCreatedTimeWithTimestamp:self.moment.timestamp]];
    string.font = WXMomentContentInnerFont;
