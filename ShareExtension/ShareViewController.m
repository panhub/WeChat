...
- (void)mainViewButtonClicked:(SEButton *)button {
    ...
    if (button.type == SEButtonTypeSession) {
    ...
    } else if (button.type == SEButtonTypeMoment) {
        ...
        [self.indicatorView startAnimatingDelay:1.f eventHandler:^{
            ...
            NSArray *array = [userDefaults arrayForKey:@"com.ext.share.to.favorites"];
            if (array) {
                __block BOOL isExists = NO;
                NSString *url = webpage[SEShareWebpageUrlKey];
                [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    if ([url isEqualToString:obj[SEShareWebpageUrlKey]]) {
                        isExists = YES;
                        *stop = YES;
                    }
                }];
            ...
            [userDefaults setObject:items.copy forKey:@"com.ext.share.to.favorites"];
...
- (void)navigationBarRightBarButtonClicked:(SENavigationBar *)navigationBar {
    ...
    [self.indicatorView startAnimatingDelay:1.f eventHandler:^{
        NSUserDefaults *userDefaults = [[NSUserDefaults alloc] initWithSuiteName:@"group.com.mn.chat.share"];
        NSArray *array = [userDefaults arrayForKey:@"com.ext.share.to.moment"];
        if (!array) array = @[];
        NSMutableArray *items = array.mutableCopy;
        NSMutableDictionary *dic = @{}.mutableCopy;
        [dic setObject:weakself.mainView.jsonValue forKey:@"com.ext.share.webpage"];
        [dic setObject:weakself.momentView.text forKey:@"com.ext.share.moment.text"];
        [items addObject:dic];
...
- (void)sessionViewDidSelectSession:(SESession *)session {
    __weak typeof(self) weakself = self;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"确认发送给:" message:session.notename preferredStyle:UIAlertControllerStyleAlert];
    [alertController addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
    [alertController addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [weakself.indicatorView startAnimatingDelay:1.f eventHandler:^{
            NSUserDefaults *userDefaults = [[NSUserDefaults alloc] initWithSuiteName:@"group.com.mn.chat.share"];
            NSArray *array = [userDefaults arrayForKey:@"com.ext.share.to.session"];
            if (!array) array = @[];
            NSMutableArray *items = array.mutableCopy;
            NSMutableDictionary *dic = @{}.mutableCopy;
            [dic setObject:weakself.mainView.jsonValue forKey:@"com.ext.share.webpage"];
...
- (void)updateImages:(NSArray <NSString *>*)imgs {
    ...
    [imgs enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (![obj hasPrefix:@"http"] || [obj.pathExtension isEqualToString:@"gif"]) return;
        img = obj.copy;
        *stop = YES;
    }];
...
- (void)fail {
    __weak typeof(self) weakself = self;
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"获取网页数据出错!" preferredStyle:UIAlertControllerStyleAlert];
