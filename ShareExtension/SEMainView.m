...
NSString * const SEShareFavoriteUrlKey = @"com.ext.share.favorite.url";
NSString * const SEShareFavoriteTitleKey = @"com.ext.share.favorite.title";
NSString * const SEShareFavoriteTimeKey = @"com.ext.share.favorite.time";
NSString * const SEShareFavoriteImageKey = @"com.ext.share.favorite.image";
...
- (void)buttonClicked:(SEButton *)button {
    if ([self.delegate respondsToSelector:@selector(mainViewButtonTouchUpInside:)]) {
        [self.delegate mainViewButtonTouchUpInside:button];
    }
}
...
- (NSDictionary *)jsonValue {
    ...
    [json setValue:self.linkLabel.text forKey:SEShareFavoriteUrlKey];
    [json setValue:self.titleLabel.text forKey:SEShareFavoriteTitleKey];
    [json setValue:[NSNumber numberWithLongLong:NSDate.date.timeIntervalSince1970].stringValue forKey:SEShareFavoriteTimeKey];
    if (self.imageView.image) {
        NSData *imageData = UIImagePNGRepresentation(self.imageView.image);
        if (imageData) [json setValue:imageData forKey:SEShareFavoriteImageKey];
    }
    return json.copy;
}
